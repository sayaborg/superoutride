...
Sound starts on the first interaction. SOUND toggles mute and VOL adjusts volume. ENGINE A/B selects WAVEGUIDE or lightweight LOOP while driving. ENGINE TUNING exposes the same seven provisional controls as the audition page; releasing a slider applies it after a short fade. Reset restores shared defaults. WAVEGUIDE is the adopted reference; LOOP is a reduced comparison. The seventh control adjusts LOOP path length only. Player and nearest-rival engines plus independent front/rear player tire sounds are synthesized without recordings. Wind remains deferred. The tire audition is at `http://localhost:8000/tools/tire-browser.html`.
