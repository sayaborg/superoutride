...
export function createM5Bike(guide, height, surfaces, s = 45) {
...
export function updateM5Bike(guide, height, surfaces, bike, input, dt, profile = M5_BIKE_PROFILE) {
    ...
    bike.course = locateWorldOnGuideLocal(guide, { x: bike.x, z: bike.z }, bike.course.segmentIndex, 3, false);
...
export function adoptM5BikeKinematics(target, source) {
    copyCommon(target, source);
    target.bankAngle = clamp(source.sprungRoll / 0.55, -1, 1) * M5_BIKE_PROFILE.maxBank;
    target.bankRate = 0;
}
export function adoptM5CarKinematics(target, source) {
    copyCommon(target, source);
    target.sprungRoll = source.sprungRoll;
}
function copyCommon(target, source) {
    target.x = source.x;
    target.y = source.y;
    target.z = source.z;
