...
export function sampleGroundMap(s, l, profile, cliffSection) {
    const abs = Math.abs(l);
    if (abs <= 0.07 && ((s % 12) + 12) % 12 < 7)
    ...
    if (leftShoulder || rightShoulder)
        return GROUND_COLORS.shoulder;
    const checker = (Math.floor(s / 3) + Math.floor(Math.abs(l) / 2)) & 1;
    if (cliffSection && l < 0)
        return checker ? GROUND_COLORS.rockA : GROUND_COLORS.rockB;
