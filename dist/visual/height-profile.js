...
export class CyclicHeightProfile {
    courseLength;
    nodes;
    constructor(courseLength, nodes) {
        this.courseLength = courseLength;
        if (!(courseLength > 0))
            throw new RangeError('course length must be > 0');
        if (nodes.length < 2)
            throw new Error('height profile requires at least two nodes');
        const copied = nodes.map((node) => ({ ...node })).sort((a, b) => a.s - b.s);
        if (Math.abs(copied[0].s) > 1e-9)
            throw new Error('height profile must start at s=0');
        for (let i = 0; i < copied.length; i += 1) {
            const n = copied[i];
            if (n.s < 0 || n.s >= courseLength)
                throw new RangeError('height node outside course');
            if (i > 0 && n.s <= copied[i - 1].s)
                throw new Error('height nodes must be unique');
        }
        this.nodes = copied;
    }
    sampleRender(s) {
...
export function createM3DebugHeightProfile(courseLength) {
