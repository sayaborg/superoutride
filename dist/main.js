...
import { createM5CameraRig, resetM5CameraRig, updateM5Camera } from './dev/m5-camera.js';
import { InputManager } from './input/input-manager.js';
import { createM5RecoveryState, recoverM5Vehicle, updateM5Recovery } from './gameplay/recovery.js';
import { createM5Car, updateM5Car } from './physics/car-physics.js';
import { adoptM5BikeKinematics, adoptM5CarKinematics, createM5Bike, updateM5Bike } from './physics/motorcycle-physics.js';
...
const canvas = mustGet('game');
const steeringPad = mustGet('steering-pad');
const throttleButton = mustGet('throttle-button');
const brakeButton = mustGet('brake-button');
...
let input = { steering: 0, throttle: false, brake: false };
window.addEventListener('keydown', (event) => {
    ...
    if (vehicleKind === 'car') {
        ...
        adoptM5CarKinematics(car, vehicle);
        vehicle = car;
        vehicleKind = 'car';
    }
});
...
let camera = updateM5Camera(cameraRig, guide, heightProfile, vehicle, cameraProfile, SIM_DT);
function frame(now) {
    ...
    while (accumulator >= SIM_DT) {
        ...
        if (vehicleKind === 'car')
            updateM5Car(guide, heightProfile, surfaceMap, vehicle, input, SIM_DT);
        else
            updateM5Bike(guide, heightProfile, surfaceMap, vehicle, input, SIM_DT);
...
function render() {
    const stats = renderM5Driving(framebuffer, farBackground, guide, camera, vehicle, terrainProfile, groundProfile, worldSprites, spriteAssets, vehicleKind);
    ctx.putImageData(imageData, 0, 0);
    const playerProjection = pseudoProject({ x: vehicle.x, y: vehicle.y, z: vehicle.z, s: vehicle.course.s }, camera);
    ...
    const bankDeg = vehicleKind === 'bike' ? vehicle.bankAngle * 180 / Math.PI : vehicle.sprungRoll * 180 / Math.PI;
