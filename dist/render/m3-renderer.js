...
import { generateTerrainLines } from '../road/terrain-line.js';
import { rgba } from './software-surface.js';
...
export function renderM3VisualCore(target, background, guide, camera, vehicle, terrainProfile, groundProfile) {
    ...
    for (let y = py - 12; y < py; y += 1)
        target.fillSpan(y, px - 5, px + 5, PLAYER_COLOR);
    for (let y = py - 10; y < py - 6; y += 1)
        target.fillSpan(y, px - 3, px + 3, PLAYER_DARK);
    let overdrawRows = 0;
