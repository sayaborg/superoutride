...
export function createM4DebugWorldSprites(guide, height, assets) {
    const authored = [];
    let serial = 0;
    for (let s = 20; s < guide.length; s += 28) {
        if (s >= 455 && s <= 625) {
            ...
            if ((Math.floor(s / 28) & 1) === 0)
                authored.push(place(`TREE_R_${serial++}`, s + 8, 10.5, assets.tree));
        }
    }
