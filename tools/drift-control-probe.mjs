...
export function createFlatProbe({
...
export function forkProbe(probe) {
  const copy = createFlatProbe({
    profile: probe.vehicle.profile,
    calibration: probe.vehicle.tireFrictionCalibration,
    torqueProtection: probe.vehicle.torqueProtection,
  });
  for (const [key, descriptor] of Object.entries(Object.getOwnPropertyDescriptors(probe.vehicle))) {
    if ('value' in descriptor) {
      copy.vehicle[key] = ['profile', 'tireFrictionCalibration', 'torqueProtection'].includes(key)
        ? descriptor.value
...
export function cycleInput(t, { direction = 1 } = {}) {
  ...
  const throttle = t < 3 ? 0.2 : t < 6 ? 0.2 + (0.6 * (t - 3)) / 3 : 0.8;
  return directInput(direction * 0.35, throttle);
}
...
export function observeProbe(probe, t) {
  ...
  return {
    ...
    requestedDriveTorque: v.powertrain.outputDriveTorque,
    deliveredDriveTorque: v.control.deliveredDriveTorque,
    frontLoad: v.frontNormalLoad,
    rearLoad: v.rearNormalLoad,
    frontOmega: v.frontWheelOmega,
    rearOmega: v.rearWheelOmega,
    frontSx: f.sx,
    frontSy: f.sy,
    rearSx: r.sx,
    rearSy: r.sy,
    ...
    rearFx: r.fx,
    rearFy: r.fy,
    frontCapacityX: f.capacityX,
    frontCapacityY: f.capacityY,
    rearCapacityX: r.capacityX,
    rearCapacityY: r.capacityY,
    frontRho: f.rho,
    rearRho: r.rho,
    ...
    rearLocked: v.control.rearWheelLocked,
    x: v.x,
    z: v.z,
  };
}
  ...
  for (let tick = 0; tick < Math.round(seconds * hz); tick++) {
    ...
    if ((tick + 1) % (hz / 10) === 0)
      rows.push({
        ...
        curvature:
          Math.hypot(vx, vz) > 0
            ? (vz * (v.velocityX - vx) - vx * (v.velocityZ - vz)) / dt / Math.hypot(vx, vz) ** 3
            : null,
        steeringInput: input.steering,
        throttleInput: Number(input.throttle),
        brakeInput: Number(input.brake),
        steeringActuator: v.actuator.steering,
        throttleActuator: v.actuator.throttle,
        brakeActuator: v.actuator.brake,
      });
  }
  return {
    hz,
    seconds,
    distance,
    initialSpeed: start.speed,
    exitSpeed: v.speed,
    forwardDisplacement: v.z - start.z,
    netDisplacement: Math.hypot(v.x - start.x, v.z - start.z),
    maxAbsBeta,
    minSpeed,
    brakeTicks,
    rearLockTicks,
...
export function runThrottleSweep(
  probe,
  {
    hz = 60,
    direction = 1,
  ...
  if (!levels.length || !levels.every((x) => Number.isFinite(x) && x >= 0 && x <= 1))
    throw new RangeError('invalid throttle sweep');
  const points = [
    ...levels.map((th) => ({ th, leg: 'up' })),
    ...[...levels].reverse().map((th) => ({ th, leg: 'down' })),
  ];
  const samples = [];
  for (const p of points) {
    const trace = runProbe(probe, dwell, () => directInput(direction * steering, p.th), { hz });
    const { rows, ...metrics } = trace;
    samples.push({ ...p, metrics, window: summarizeWindow(trace, Math.max(0.1, dwell - 1), dwell), rows });
  }
  return {
    meaning: 'finite-time hysteresis, not steady equilibria or a continuity proof',
    hz,
    dwell,
    steering,
    samples,
...
async function main() {
  ...
  const calibration = characteristics
    ? createArcadeTireFrictionCalibration(compileTireCharacteristics(characteristics))
  ...
  const report = {
    ...
    direction,
    initialKmh: speed,
    characteristics: readTireCharacteristics(calibration.front),
    trace,
  };
    ...
    JSON.stringify(
      {
        ...
        trace: {
          ...summary,
          ...(samples ? { windows: samples.map(({ th, leg, window }) => ({ th, leg, window })) } : {}),
        },
      },
      null,
      2,
