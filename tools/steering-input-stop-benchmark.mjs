/** Same-input microbenchmark against the explicit six-argument M9.26 diagnostic baseline. */
...
if(!process.argv[2]) throw new Error('usage: node tools/steering-input-stop-benchmark.mjs M926_DIST [OUT_JSON]');
const {limitSteeringInput:current}=await import(pathToFileURL(resolve(process.argv[2],'physics/steering-input-limiter.js')).href);
import {limitSteeringInput as redesigned} from '../dist/physics/steering-input-limiter.js';
...
for(const group of ['active','lowSpeed','mixed','neutral']){
 cases[group]=Array.from({length:1024},()=>{
  ...
  return [(rng()-.5)*.8,group==='neutral'?0:(rng()<.5?-1:1)*(.2+rng()*.14),Math.PI/3,body,
   {...c,forceTransmitting:true,tireFrameValid:true,reachVelocity:{x:speed*Math.sin(beta),y:0,z:speed*Math.cos(beta)}},tire];
 });
}
const fns={current:(a,d,m,b,c,t)=>current(a,d,m,b,c,t),redesigned:(a,d,m,b,c,t)=>redesigned(a,d,b,c,t)},count=500000,rounds=11,report={node:process.version,arch:os.arch(),cpu:os.cpus()[0].model,count,rounds,groups:{}};
let checksum=0;
