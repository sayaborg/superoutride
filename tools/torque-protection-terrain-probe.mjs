...
export const TERRAIN_CASES = Object.freeze({
  lowGripDrive: Object.freeze({ grip: 0.25, kind: 'drive', speed: 15, seconds: 6 }),
  lowGripBrake: Object.freeze({ grip: 0.25, kind: 'brake', speed: 30, seconds: 10 }),
  uphillDrive: Object.freeze({ grade: 0.1, kind: 'drive', speed: 15, seconds: 6 }),
  downhillBrake: Object.freeze({ grade: -0.1, kind: 'brake', speed: 30, seconds: 8 }),
  gripDropBrake: Object.freeze({
    grip: 1,
    gripAfter: 0.25,
    changeS: START + 35,
    kind: 'brake',
  ...
  lowGripReversal: Object.freeze({ grip: 0.25, kind: 'reversal', speed: 30, seconds: 6 }),
...
export function createTerrainProbe(entry, options = {}) {
  ...
  if (
    ![grip, gripAfter, changeS, grade, speed].every(Number.isFinite) ||
    grip < 0 ||
    gripAfter < 0 ||
    changeS < 0 ||
    changeS > LENGTH ||
    speed < 0 ||
    !['flat', 'crest'].includes(terrain) ||
    (!(typeof calibration === 'object' && calibration !== null) && !['browser', 'stock'].includes(calibration)) ||
    typeof protectedRun !== 'boolean' ||
    (terrain === 'crest' && grade !== 0)
  )
    throw new RangeError('invalid terrain probe configuration');
  ...
  const base = new SurfaceMap(LENGTH, [
    { sStart: 0, name: 'Terrain protection diagnostic', bands: [{ lMin: -500, lMax: 500, type: 'ASPHALT' }] },
  ]);
  ...
  const surface = Object.freeze({
    ...
    sample(s, l) {
      ...
      return {
        ...value,
        material: { ...value.material, gripFactor: value.material.supported ? (s >= changeS ? gripAfter : grip) : 0 },
      };
    },
  });
...
export function terrainInput(t, kind, direction = 1) {
  ...
  if (t < 0.5 || kind === 'coast') return neutral;
  if (kind === 'drive') return { ...neutral, throttle: true };
  if (kind === 'brake') return { ...neutral, brake: true };
  if (kind === 'turnDrive') return { steering: direction * 0.35, throttle: t >= 1.5, brake: false };
  if (kind === 'turnBrake') return { steering: direction * 0.35, throttle: false, brake: t >= 1.5 };
  if (kind === 'reversal') return { steering: direction * (t < 1.5 ? 0.35 : -0.35), throttle: false, brake: t >= 1.5 };
...
export function runTerrainProbe(entry, options = {}) {
  ...
  if (
    ![60, 120, 240].includes(hz) ||
    !(seconds > 0) ||
    !Number.isFinite(seconds) ||
    !['drive', 'brake', 'turnDrive', 'turnBrake', 'reversal', 'coast'].includes(kind) ||
    ![-1, 1].includes(direction)
  )
    throw new RangeError('invalid terrain probe run');
  const p = createTerrainProbe(entry, fixture),
    v = p.vehicle;
  const out = {
    id: entry.profile.id,
    hz,
    ...
    kind,
    direction,
    calibration: fixture.calibration ?? 'browser',
    protectedRun: fixture.protectedRun ?? true,
    seconds: 0,
    completed: false,
    error: null,
    overturned: false,
    ...
    maxAbsBeta: 0,
    maxAbsMovingBeta: 0,
    speedAtMaxMovingBeta: null,
    timeAtMaxMovingBeta: null,
    maxAbsBetaAbove15: 0,
    maxAbsYawRate: 0,
    maxAbsPitch: Math.abs(v.pitch) * DEG,
    frontLiftTime: 0,
    rearLiftTime: 0,
    airborneTime: 0,
    recontacts: 0,
    supportLimitedTime: 0,
    infeasibleTime: 0,
    infeasibleNonzeroTorque: 0,
    torqueBudgetViolation: 0,
    maxEllipse: 0,
    maxPositiveSlipPower: 0,
    zeroLoadForceViolation: 0,
    ...
    try {
      ...
      if (
        ![
          ...
          v.rearWheelOmega,
          v.frontSteerAngle,
          v.powertrain.engineRpm,
        ].every(Number.isFinite)
      )
        throw new Error('nonfinite mechanics');
      out.seconds = (tick + 1) / hz;
      out.finalSpeed = v.speed;
      out.minSpeed = Math.min(out.minSpeed, v.speed);
      ...
      if (v.speed > 5 && Math.abs(beta) > out.maxAbsMovingBeta) {
        out.maxAbsMovingBeta = Math.abs(beta);
        out.speedAtMaxMovingBeta = v.speed;
        out.timeAtMaxMovingBeta = out.seconds;
      }
      ...
      if (!c.supportFeasible)
        out.infeasibleNonzeroTorque = Math.max(
          out.infeasibleNonzeroTorque,
          Math.abs(c.frontDriveTorque) +
            Math.abs(c.rearDriveTorque) +
            Math.abs(c.frontBrakeTorque) +
            Math.abs(c.rearBrakeTorque),
        );
      for (const side of ['Front', 'Rear'])
        for (const axis of ['Drive', 'Brake']) {
          const got = c[side.toLowerCase() + axis + 'Torque'],
            requested = c['requested' + side + axis + 'Torque'];
      ...
      out.torqueBudgetViolation = Math.max(
        out.torqueBudgetViolation,
      ...
      const forces = [f, r].map((contact, i) => {
        ...
        out.maxPositiveSlipPower = Math.max(
          out.maxPositiveSlipPower,
          -force.referenceSpeed * (force.fx * force.sx + force.fy * force.sy),
        );
        if (!contact.normalLoad || !contact.tireFrameValid)
          out.zeroLoadForceViolation = Math.max(out.zeroLoadForceViolation, Math.abs(force.fx), Math.abs(force.fy));
      ...
      previousGrip = grip;
      if (capture && (tick + 1) % (hz / 20) === 0)
        out.rows.push({
          t: out.seconds,
          speed: v.speed,
          beta,
          pitch: v.pitch * DEG,
          pitchRate: v.pitchRate,
          yawRate: v.yawRate,
          frontGap: f.gap,
          rearGap: r.gap,
          frontLoad: f.normalLoad,
          rearLoad: r.normalLoad,
          frontSx: forces[0].sx,
          rearSx: forces[1].sx,
          grip,
          ...c,
        });
      out.overturned = [f, r].some(
        (contact) =>
          body.up.x * contact.surface.normal.x +
            body.up.y * contact.surface.normal.y +
            body.up.z * contact.surface.normal.z <=
          0,
      );
...
async function main() {
  ...
  for (const entry of VEHICLE_CATALOG)
    for (const [name, options] of Object.entries(TERRAIN_CASES)) {
      ...
      JSON.stringify(
        {
          ...
          calibration,
          observation:
            'Outer-tick post-update contact samples; control telemetry is the final internal substep. Not continuous-time certification.',
          rows,
        },
        null,
        2,
