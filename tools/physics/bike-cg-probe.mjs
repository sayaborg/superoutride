...
export async function compareBikeCg(baselinePath, ratios = [null, 0.3, 0.25, 0.2], rates = [120]) {
  ...
  for (const entry of VEHICLE_CATALOG.filter((e) => e.presentationFamily === 'BIKE')) {
    const seed = Object.entries(seeds).find(
      ([key, value]) => key.endsWith('_AUTHORING') && value.id === entry.profile.id,
    )[1];
    for (const ratio of ratios) {
      ...
      for (const hz of rates)
        for (const grip of [1, 0.25])
          for (const kind of ['brake', 'turnBrake', 'reversal']) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  if (process.argv.length !== 4) throw new Error('usage: node tools/physics/bike-cg-probe.mjs BASELINE_DIST OUT_JSON');
  const rows = await compareBikeCg(process.argv[2], [null, 0.3, 0.25, 0.2], [60, 120, 240]);
  await writeFile(
    process.argv[3],
    JSON.stringify(
      {
        ...
        scope:
          'CG-only matched authoring, independent equilibrium starts; no recovery, force or state correction. Not global handling acceptance.',
        rows,
      },
      null,
      2,
