...
if (!process.argv[2])
  throw new Error('usage: node tools/physics/steering-input-stop-benchmark.mjs REFERENCE_DIST [OUT_JSON]');
const { limitSteeringInput: baseline } = await import(
  pathToFileURL(resolve(process.argv[2], 'physics/steering-input-limiter.js')).href
);
...
for (const group of ['active', 'lowSpeed', 'mixed', 'neutral']) {
  cases[group] = Array.from({ length: 1024 }, () => {
    const beta = (rng() - 0.5) * (group === 'mixed' ? 5 : 0.8),
      speed = group === 'lowSpeed' ? 0.001 + rng() * 0.03 : 10 + rng() * 60;
    return [
      (rng() - 0.5) * 0.8,
      group === 'neutral' ? 0 : (rng() < 0.5 ? -1 : 1) * (0.2 + rng() * 0.14),
      body,
      {
        ...c,
...
const fns = { baseline, current },
  count = 500000,
  rounds = 11,
  report = { node: process.version, arch: os.arch(), cpu: os.cpus()[0].model, count, rounds, groups: {} };
let checksum = 0;
for (const [group, args] of Object.entries(cases)) {
  ...
  report.groups[group] = Object.fromEntries(
    Object.entries(times).map(([k, v]) => [
      k,
      { medianMs: [...v].sort((a, b) => a - b)[5], minMs: Math.min(...v), maxMs: Math.max(...v), samples: v },
    ]),
  );
}
