import { vehicleUpdateForBuild } from '../build/build-contract.mjs';
...
export const STEERING_STOP_CASES = Object.freeze([
  { name: 'smallCoast', u: 0.15 },
  { name: 'halfCoast', u: 0.5 },
  { name: 'fullCoast', u: 1 },
  { name: 'fullThrottle', u: 1, pedal: 'throttle' },
  { name: 'fullBrakeHold', u: 1, pedal: 'brake' },
  { name: 'fullBrakeRelease', u: 1, pedal: 'brake', release: 1.5 },
  { name: 'reversal', u: 1, reverse: 1.5 },
]);
export function runSteeringStopCase(parent, scenario, { hz = 120, direction = 1, update = updateArcadeVehicle } = {}) {
  ...
  for (let i = 0; i < 6 * hz; i++) {
    const t = i * dt,
      steering =
        t < 0.5 || t >= 3 || t >= (scenario.release ?? Infinity)
          ? 0
          : direction * scenario.u * (t >= (scenario.reverse ?? Infinity) ? -1 : 1);
    const input = {
      steering,
      throttle: scenario.pedal === 'throttle' && t >= 1.5 && t < 3 ? 1 : 0,
      brake: scenario.pedal === 'brake' && t >= 1.5 && t < 3 ? 1 : 0,
    };
    ...
    if (t >= 0.5 && t < 1.5 && speed > 1) earlyLateral += (vz * (v.velocityX - vx) - vx * (v.velocityZ - vz)) / speed;
    cutSeconds += Math.abs(c.requestedSteerOffset - c.deliveredSteerOffset) > 1e-6 ? dt : 0;
    maxInputStepDeg = Math.max(maxInputStepDeg, Math.abs(c.deliveredSteerOffset - lastE) * DEG);
    lastE = c.deliveredSteerOffset;
    unsupportedSeconds += !v.supported ? dt : 0;
    if (i === 3 * hz - 1) at3 = { kmh: v.speed * 3.6, absBeta: beta };
  }
  return {
    earlyMeanLateralAcceleration: earlyLateral,
    peakBetaAbove5,
    recoveryBetaAbove5,
    cutSeconds,
    maxInputStepDeg,
...
export function runMatchedSteeringBrake(parent, update = updateArcadeVehicle, hz = 120) {
  ...
  for (let i = 0; i < hz * 4.5; i++) {
    ...
    const beta = Math.abs(Math.atan2(v.lateralSpeed, v.longitudinalSpeed) * DEG);
    if (v.speed > 15 && beta > peakBetaAbove15) {
      peakBetaAbove15 = beta;
      peakAt = { t: (i + 1) / hz, kmh: v.speed * 3.6 };
    }
  }
...
async function main() {
  ...
  const baseline = baselinePath
    ? (await import(pathToFileURL(resolve(baselinePath, 'physics/arcade-vehicle-physics.js')).href)).updateArcadeVehicle
    : null;
  const pairs = [];
  for (const entry of VEHICLE_CATALOG)
    for (const grip of [1, 0.25])
      for (const d of [12, 20])
        for (const scenario of STEERING_STOP_CASES) {
          const parent = createTerrainProbe(entry, { grip, speed: 30 });
          setArcadeVehicleSteeringOffsetMax(parent.vehicle, d / DEG);
          pairs.push({
            id: entry.profile.id,
            grip,
            d,
            scenario: scenario.name,
            current: runSteeringStopCase(parent, scenario),
            ...(baseline ? { baseline: runSteeringStopCase(parent, scenario, { update: baseline }) } : {}),
          });
        }
  const refinements = [];
  for (const hz of [60, 120, 240])
    for (const direction of [-1, 1]) {
      ...
      setArcadeVehicleSteeringOffsetMax(parent.vehicle, 20 / DEG);
      const scenario = STEERING_STOP_CASES.find((x) => x.name === 'fullBrakeHold');
      refinements.push({
        hz,
        direction,
        current: runSteeringStopCase(parent, scenario, { hz, direction }),
        ...(baseline ? { baseline: runSteeringStopCase(parent, scenario, { hz, direction, update: baseline }) } : {}),
      });
    }
  const parent = createTerrainProbe(
    VEHICLE_CATALOG.find((e) => e.profile.id === 'VFR750R'),
    { grip: 0.25, speed: 30 },
  );
  ...
  const matched = [60, 120, 240].map((hz) => ({
    hz,
    current: runMatchedSteeringBrake(parent, updateArcadeVehicle, hz),
    ...(baseline ? { baseline: runMatchedSteeringBrake(parent, baseline, hz) } : {}),
  }));
  const report = {
    node: process.version,
    scope:
      'Fixed scripted inputs, not a force optimum or handling certification. Velocities in m/s except kmh; angles in degrees. Final substep control sampling.',
    pairs,
    refinements,
    matched,
  };
