...
export function brakingInput(t, action, { direction = 1, correctionSeconds = 0.3, applyMode = 'RATE_LIMITED' } = {}) {
  if (
    !Number.isFinite(t) ||
    t < 0 ||
    !BRAKING_ACTIONS.includes(action) ||
    ![-1, 1].includes(direction) ||
    !Number.isFinite(correctionSeconds) ||
    correctionSeconds < 0 ||
    !['RATE_LIMITED', 'DIRECT'].includes(applyMode)
  )
    throw new RangeError('invalid braking input schedule');
  let steering = 0.35,
    brake = 0;
  if (action === 'holdBrake25') brake = 0.25;
  if (action === 'holdBrake50') brake = 0.5;
  if (['holdBrake100', 'releaseBrake', 'delayedReleaseBrake', 'counterPulseBrake', 'reverseBrake'].includes(action))
    brake = 1;
  if (['releaseCoast', 'releaseBrake'].includes(action)) steering = 0;
  if (action === 'delayedReleaseBrake') steering = t < correctionSeconds ? 0.35 : 0;
  if (action === 'counterPulseBrake') steering = t < correctionSeconds ? -0.35 : 0;
  if (['reverseCoast', 'reverseBrake'].includes(action)) steering = -0.35;
  return {
...
export function brakingStateFingerprint(vehicle) {
  const values = Object.fromEntries(
    Object.entries(Object.getOwnPropertyDescriptors(vehicle))
...
export function observeBrakingState(probe, t) {
  ...
  const station = (side, profile, steer, omega) => {
    ...
    const f = evaluateTireForce(
      ...
      c.longitudinalVelocity,
      c.lateralVelocity,
      c.tireFrameValid ? c.normalLoad : 0,
    ...
    return {
      ...
      q: c.q,
      qDot: c.qDot,
      vx: c.longitudinalVelocity,
      vy: c.lateralVelocity,
      omega,
      sx: f.sx,
      sy: f.sy,
      fx: f.fx,
      fy: f.fy,
      capacityX: f.capacityX,
      capacityY: f.capacityY,
      normalizedFx: valid ? f.fx / f.capacityX : null,
      normalizedFy: valid ? f.fy / f.capacityY : null,
      ellipse: valid ? (f.fx / f.capacityX) ** 2 + (f.fy / f.capacityY) ** 2 : 0,
      dissipatedSlipPower: f.referenceSpeed * (f.fx * f.sx + f.fy * f.sy),
      yawX,
      yawY,
      yawNormal,
      yawContact: yawX + yawY + yawNormal,
  ...
  return {
    ...
    yawRate: v.yawRate,
    pitch: v.pitch * DEG,
    steer: v.frontSteerAngle * DEG,
    automaticLimited:
      Math.abs(vehicleBodyTravelDirection(body, v.profile.steeringLowSpeedRegularization)) >=
...
export function decomposeContactYawChange(before, after) {
  let loadContribution = 0,
    responseAndGeometryContribution = 0;
  for (const side of ['front', 'rear']) {
    const a = before[side],
      b = after[side];
    if (!(a.load > 0 && b.load > 0)) return null;
    const ma = a.yawContact / a.load,
      mb = b.yawContact / b.load;
  ...
  return {
    delta,
    loadContribution,
    responseAndGeometryContribution,
    residual: delta - loadContribution - responseAndGeometryContribution,
  };
}

export function runBrakingComparison(
  entry,
  {
    hz = 120,
    grip = 1,
    ...
    speed = 30,
    seconds = 4.5,
    correctionSeconds = 0.3,
    calibration = 'browser',
    steeringCalibration,
    applyMode = 'RATE_LIMITED',
    actions = BRAKING_ACTIONS,
    capture = false,
  } = {},
) {
  if (
    ...
    seconds <= 0 ||
    !Number.isInteger(seconds * hz) ||
    !Array.isArray(actions) ||
    actions.length === 0 ||
    new Set(actions).size !== actions.length
  )
    throw new RangeError('invalid braking comparison domain');
  ...
  for (let tick = 0; tick < hz * 1.5; tick++) {
    const input = {
      ...terrainInput(tick / hz, 'turnBrake', direction),
      steeringApplyMode: applyMode,
      pedalApplyMode: applyMode,
    };
    updateArcadeVehicle(
  ...
  for (const action of actions) {
    const probe = forkProbe(parent),
      v = probe.vehicle;
    if (
      probe.guide !== parent.guide ||
      probe.height !== parent.height ||
      probe.surface !== parent.surface ||
      brakingStateFingerprint(v) !== fingerprint
    )
      throw new Error('comparison fork changed reached state/world');
    const out = {
      action,
      ...
      finalSpeed: initial.speed,
      minimumSpeed: initial.speed,
      maxAbsBetaAbove15: initial.speed > 15 ? Math.abs(initial.beta) : null,
      peakAbove15: initial.speed > 15 ? initial : null,
      distance: 0,
      forwardDisplacement: 0,
      maxAbsYawRate: Math.abs(initial.yawRate),
      firstBeta15: null,
      firstBeta45: null,
      firstBeta90: null,
      firstAutomaticLimit: null,
      lockTimeBefore45: 0,
      liftTime: 0,
      infeasibleTime: 0,
      maxTorqueBudgetViolation: 0,
      maxEllipse: 0,
      maxPositiveSlipPower: 0,
      samples: [],
      rows: capture ? [initial] : undefined,
    };
    for (let tick = 0; tick < seconds * hz; tick++) {
      ...
      try {
        ...
        const x = observeBrakingState(probe, (tick + 1) / hz),
          c = x.control;
        if (!x.front.upright || !x.rear.upright) throw new Error('overturned');
        out.seconds = x.t;
        out.finalSpeed = x.speed;
        out.minimumSpeed = Math.min(out.minimumSpeed, x.speed);
        out.distance += x.speed / hz;
        out.forwardDisplacement = x.z - initial.z;
        ...
        if (x.speed > 15) {
          if (out.maxAbsBetaAbove15 === null || Math.abs(x.beta) > out.maxAbsBetaAbove15) {
            out.maxAbsBetaAbove15 = Math.abs(x.beta);
            out.peakAbove15 = x;
          }
          for (const angle of [15, 45, 90])
            if (Math.abs(x.beta) >= angle && out['firstBeta' + angle] === null) out['firstBeta' + angle] = x;
        ...
        out.liftTime += Number((x.front.gap > 1e-5 && x.rear.load > 0) || (x.rear.gap > 1e-5 && x.front.load > 0)) / hz;
        ...
        if ([0.1, 0.2, 0.3, 0.5, 1].some((t) => tick + 1 === Math.round(t * hz)))
          out.samples.push({ ...x, contactYawChange: decomposeContactYawChange(initial, x) });
...
async function main() {
  ...
  if (entries.length === 0 || (args.includes('--id') && id === undefined))
    throw new RangeError('unknown/missing vehicle id');
  const comparisons = [];
    ...
    for (const grip of [1, 0.25]) {
      ...
      console.log(
        JSON.stringify({
          id: entry.profile.id,
          grip,
          outcomes: comparison.results.map((r) => ({
            action: r.action,
            completed: r.completed,
            maxAbsBetaAbove15: r.maxAbsBetaAbove15,
            error: r.error,
          })),
        }),
      );
      ...
      JSON.stringify(
        {
          ...
          scope:
            'Input-only matched forks; sampled post-update contact moments, not exact integrated wrench or yaw certification.',
          comparisons,
        },
        null,
        2,
