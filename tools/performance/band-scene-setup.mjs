...
import { renderBandEvidenceStill } from './band-evidence-still.mjs';
...
export async function createBandSceneProbe(mode, variant, rivals) {
  ...
  const trial =
    variant === 'resident'
      ? null
      : variant === 'revised'
        ? await createRevisedBandTrialScene(scene, course, assets)
  ...
  return {
    ...
    visualEvidence() {
      return renderBandEvidenceStill(
        scene,
        course,
        assets,
        target,
