...
export async function runHotPathProbe(buildPath = 'dist') {
  ...
  const guide = compileGuidePath(compileRasterPath([{ x: 0, z: 0 }, { x: 0, z: 10000 }]),
    { lMax: 1000, mMin: .25, dCam: 5 });
  const height = new HeightProfile(10000, [{ s: 0, y: 0 }, { s: 10000, y: 0 }]);
  ...
  for (const entry of VEHICLE_CATALOG) {
    for (let i = 0; i < 128; i++) {
      record(solveWheelOmega({ omegaPrevious: (random() - .5) * 600,
        inertia: .1 + random() * 10, rollingRadius: .1 + random(),
        longitudinalVelocity: (random() - .5) * 160, lateralVelocity: (random() - .5) * 100,
        normalLoad: i % 8 === 0 ? 0 : random() * 20000,
        gripFactor: i % 9 === 0 ? 0 : random() * 3, rollingResistance: random() * .04,
  ...
  return { sha256: hash.digest('hex'), milliseconds: performance.now() - started,
    wheelCases: VEHICLE_CATALOG.length * 128, rates: [60, 120, 240], vehicles: VEHICLE_CATALOG.length };
}
