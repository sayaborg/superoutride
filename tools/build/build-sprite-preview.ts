...
const mastersDocument = 'content/sprites/vehicles.json';
const masters = JSON.parse(await readFile(new URL(`../../${mastersDocument}`, import.meta.url), 'utf8')) as {
  sprites: SpriteLodDocument[];
  sets: Record<string, { assets: number[][]; brakeLamp: { off: number; on: number } }>;
};
// The library reader admits the masters and their set bindings; each image compiles with its set's lamp.
const compiled = admit(mastersDocument, () => {
