...
export function runProtectionProbe(
  entry,
  { hz = 120, seconds = 6, kind = 'drive', speed = 15, protectedRun = true, capture = false } = {},
) {
  if (
  ...
  const out = {
    ...
    frontLiftTime: 0,
    rearLiftTime: 0,
    minFrontLoad: Infinity,
    minRearLoad: Infinity,
    driveLimitedTime: 0,
    brakeLimitedTime: 0,
  ...
  for (let tick = 0; tick < Math.round(hz * seconds); tick++) {
    ...
    out.maxPitch = Math.max(out.maxPitch, (v.pitch * 180) / Math.PI);
    out.minPitch = Math.min(out.minPitch, (v.pitch * 180) / Math.PI);
    out.minFrontLoad = Math.min(out.minFrontLoad, f.normalLoad);
    out.minRearLoad = Math.min(out.minRearLoad, r.normalLoad);
    ...
    out.brakeLimitedTime +=
      Number(c.frontBrakeTorque + c.rearBrakeTorque < c.requestedFrontBrakeTorque + c.requestedRearBrakeTorque - 1e-6) /
      hz;
    ...
    if (capture)
      rows.push({
        ...
        beta: Math.atan2(v.lateralSpeed, v.longitudinalSpeed),
        yawRate: v.yawRate,
        frontSx: fs.sx,
        rearSx: rs.sx,
        frontSy: fs.sy,
    ...
    if (kind === 'brake' && t > 0.5 && v.speed < 0.5) break;
  }
  ...
  for (const entry of VEHICLE_CATALOG)
    for (const kind of ['drive', 'brake'])
      for (const protectedRun of [false, true]) {
        const x = runProtectionProbe(entry, { hz, kind, protectedRun });
        rows.push(x);
