...
order is bank mixing → 18 Hz DC removal → bounded soft saturation → adjustable one-pole low-pass (default 7300 Hz).
...
Game and audition use one [tuning control](../src/browser/audio-tuning-controls.ts) for five combinable sliders for outlet reflection, boundary cutoff,
propagation attenuation, closed-excitation floor and post-clip output cutoff. Displayed values and reset come directly
from the DSP's shared defaults (-1, 3100 Hz, 0.03 Np/m, 0.22 and 7300 Hz). Outlet reflection includes
...
The final output cutoff spans 100–12000 Hz in 100 Hz steps and remains independent of the
reflection-wave cutoff. It adjusts the existing post-clip one-pole (approximately -6 dB/oct),
not a second filter or the reflected waves. This is a listening approximation of muffler
high-frequency attenuation, not measured muffler transmission loss. Default values remain
waveform-identical. The selected setting can be checked across all nine vehicles and both output rates.
