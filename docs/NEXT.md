...
- **8-8b — Course rules:** course rules are mandatory, and race code receives them without non-null assertions. A course is timed exactly when its rules carry the CLASSIC settings; the build decides reference runs and time budgets from the document, and Session reads the compiled rules. The hard-coded untimed-course list and the browser's inference from delivered budgets disappear. Behavior unchanged.
...
- **9-2 — Sound graph:** generators, voices and buses that can accommodate BGM, environmental audio and effects.
  ...
  Engine voices use the powertrain's effective opening as load and play downshift blips from shift observations.
...
Add surface materials as product courses need them, with physical and sound values tuned on those courses: kerbs (circuits), gravel traps, concrete (streets, run-offs, pits), wet asphalt for rain sections, packed snow, mud and shallow water (off-road), and cobblestones. Kerbs and cobblestones rely on sound for their vibration unless a material roughness is added later.

### Pending product decisions

- Start procedure: 8-5f's held start is provisional; decide countdown lamps, rolling starts and their Session rules.
