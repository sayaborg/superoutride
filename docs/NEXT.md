...
and waveguide with provisional shared coefficients. Both expose the same five tuning sliders,
including the post-clip final LPF; the game commits changes on release. D/M/ACT use compact minus/value/plus controls with unchanged ranges. Adopt only one and remove the other using
