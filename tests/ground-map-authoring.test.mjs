...
test('a road without authored paint does not acquire an implicit center dash', () => {
  ...
  assert.equal(sampleGroundMap(3, 1, {
    ...profile, roadMarkings: [{ centerL: 1, width: 0.2, pattern: 'SOLID' }],
  }), GROUND_COLORS.marking);
});

test('outer texture follows logical material on either side, without a cliff flag', () => {
  for (const [left, right] of [['ROCK', 'GRASS'], ['GRASS', 'ROCK']]) {
    const authored = { ...profile, logical: new GroundMapLogicalProfile(100, [
      { sStart: 0, name: 'material', left, right },
    ]) };
    const expected = { ROCK: [GROUND_COLORS.rockA, GROUND_COLORS.rockB], GRASS: [GROUND_COLORS.grassA, GROUND_COLORS.grassB] };
...
test('source and stage junctions repeat the authored paint about carriageway centers', () => {
  const junction = new JunctionCrossSectionProfile({
    sWidenStart: 10, sMedianStart: 20, sSeparatedStart: 40,
    parentRoadWidth: 9, childRoadWidth: 9, finalMedianWidth: 2, shoulderWidth: 1,
  });
  ...
  for (const side of ['LEFT', 'RIGHT']) {
    ...
    assert.equal(sampleGroundMap(s, center + 1, {
      ...profile, junction, junctionMarkings: markings,
    }), GROUND_COLORS.marking);
    assert.equal(sampleStageGroundMapAtLevel(s, center + 1, 0, view, {
      ...profile, stageJunction: junction, junctionMarkings: markings,
    }), GROUND_COLORS.marking);
  }
});
...
test('current browser course and successor paint matches the immutable reference', async () => {
  ...
  const { createM72DefaultBranchingParent, M7_2_DEFAULT_BRANCHING_FORK } = await import('../dist/dev/m7-2-default-branching-highway.js');
  const { createM638DeclarativeForkGrowthRuntime } = await import('../dist/dev/m6-38-declarative-fork-growth-plan.js');
  const { createM4SpriteAssets } = await import('../dist/visual/m4-sprite-assets.js');
  const { createM3FarBackground } = await import('../dist/visual/far-background.js');
  ...
  const route = createM638DeclarativeForkGrowthRuntime(parent.guide, {
    ...parent, selectFarBackground: createM3FarBackground, worldSprites: [],
  }, createM4SpriteAssets(), M7_2_DEFAULT_BRANCHING_FORK);
  const sources = [
    ...route.registry.packages,
    ...[createM71HighwayGroundProfile(), createM93TsukubaGroundProfile(), createM96FiscoGroundProfile()]
      .map(groundProfile => ({ groundProfile, roadView: null })),
  ];
    ...
    for (let s = 0; s < length; s += 7.25) {
      for (let l = -ground.groundLeft; l <= ground.groundRight; l += 0.125) {
        if (view) {
          ...
          assert.equal(sampleStageGroundMapAtLevel(s, l, 0, view, ground),
