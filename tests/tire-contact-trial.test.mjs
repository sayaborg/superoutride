...
import { ContactMode, DistanceRoughness, TireContactTrial } from '../dist/dev/diagnostics/tire-contact-model.js';
...
test('contact midpoint has dissipative friction and a discrete energy balance without a clipper', () => {
  for (const rate of [44100, 48000, 96000]) {
    ...
    for (let i = 0; i < 12000; i++) {
      ...
      assert.ok(mode.lastIterations <= CONTACT_TRIAL.maxSolveIterations);
    }
  }
});
...
test('the two native sample rates converge toward a finer midpoint reference', () => {
  const values = [44100, 48000, 96000].map((rate) => {
    const samples = settled(new ContactMode(rate, CONTACT_TRIAL.frictionMode), rate, 0.5, 5);
    const crossings = [];
...
test('trial boundaries reject invalid tuning and own their parameter snapshots', () => {
  for (const rate of [NaN, Infinity, 0, CONTACT_TRIAL.minRate - 1])
    assert.throws(() => new TireContactTrial(rate, 42), RangeError);
  assert.throws(
    () => new ContactMode(48000, CONTACT_TRIAL.frictionMode, { ...CONTACT_TRIAL.friction, dynamic: undefined }),
    RangeError,
  );
  const mode = { ...CONTACT_TRIAL.frictionMode };
  const a = new ContactMode(48000, mode),
    b = new ContactMode(48000, mode);
  mode.massKg *= 2;
  for (let i = 0; i < 100; i++) assert.equal(a.step(0.5, 5), b.step(0.5, 5));
  const voice = new TireContactTrial(48000, 42);
  ...
  assert.throws(() => new TireContactTrial(48000, 42, 'toString'), RangeError);
...
test('rolling, sliding without travel, standstill and no support are separate causal cases', () => {
  const rolling = new TireContactTrial(48000, 42),
    spinning = new TireContactTrial(48000, 42);
  const airborne = new TireContactTrial(48000, 42),
    rest = new TireContactTrial(48000, 42);
...
test('front and rear use one model but do not share mutable oscillator or roughness state', () => {
  const a = new TireContactTrial(48000, CONTACT_TRIAL.frontSeed);
  const b = new TireContactTrial(48000, CONTACT_TRIAL.rearSeed);
  const replay = new TireContactTrial(48000, CONTACT_TRIAL.rearSeed);
  a.update(30, 0.5, 5);
  b.update(30, 0.5, 5);
...
test('both textures retain finite unclipped output and decay after abrupt transitions at input corners', () => {
  for (const rate of [44100, 48000, 96000])
    for (const texture of Object.keys(CONTACT_TEXTURES)) {
      const voice = new TireContactTrial(rate, 42, texture);
