...
import { deriveVehicleLeanRadians as lean, deriveVehicleNormalizedBank as bank } from '../dist/render/vehicle-presentation.js';
...
import { createM4SpriteAssets, selectVehicleSprite } from '../dist/visual/m4-sprite-assets.js';
...
test('M9.28 four bikes lower only compiled CG/free reach to 30% wheelbase; five cars are exact M9.27', () => {
  ...
  const withoutCg = p => JSON.parse(JSON.stringify(p, (k,v) => ['desiredCgHeight','freeReachDown'].includes(k) ? undefined : v));
  ...
  for (const { profile: p } of VEHICLE_CATALOG) {
    if (p.id in carHashes) {
      ...
      assert.ok(Math.abs(p.desiredCgHeight / (p.frontAxle + p.rearAxle) - .3) < 1e-14);
      ...
      const probe = createTerrainProbe(VEHICLE_CATALOG.find(e=>e.profile===p),{speed:0});
      assert.ok(Math.abs(probe.vehicle.presentationY)<1e-12,'lower CG preserves grounded sprite placement');
    }
  }
});

for (const hz of [60,120,240]) test(`M9.28 lower CG reduces moving yaw excursion in all-four matched turn-brake cases at ${hz}Hz`, () => {
  for (const entry of bikes) for (const grip of [1,.25]) {
    const options={hz,grip,kind:'turnBrake',speed:30,seconds:6};
    const old=runTerrainProbe(withM927BikeCgEntry(entry),options), current=runTerrainProbe(entry,options);
    assert.equal(current.error,null);assert.equal(current.completed,true);assert.equal(current.overturned,false);
    assert.ok(current.maxAbsBetaAbove15<old.maxAbsBetaAbove15,JSON.stringify({id:entry.profile.id,hz,grip,old:old.maxAbsBetaAbove15,current:current.maxAbsBetaAbove15}));
    assert.ok(current.maxEllipse<=1+1e-10);assert.equal(current.maxPositiveSlipPower,0);
    assert.equal(current.torqueBudgetViolation,0);assert.equal(current.zeroLoadForceViolation,0);
  }
});

test('M9.28 lateral G owns lean, including sliding sign, upright spin and sprite saturation', () => {
  assert.equal(lean({}),0);
  for (const value of [-2.5,-1,-.5,0,.5,1,2.5]) {
...
test('M9.28 real sideslip acceleration and world sprite bank share the observed outer-step G', () => {
  const p=createTerrainProbe(bikes[0],{speed:30}),v=p.vehicle,dt=1/120;
  ...
  updateArcadeVehicle(p.guide,p.height,p.surface,v,{steering:0,throttle:0,brake:0},dt);
  const right=arcadeBodyKinematics(v).right;
  const a=((v.velocityX-before.x)*right.x+(v.velocityY-before.y)*right.y+(v.velocityZ-before.z)*right.z)/dt;
  assert.ok(Math.abs(a)>1);
  assert.ok(Math.abs(lean(v)-Math.atan2(a,g))<1e-12);
  assert.ok(Math.abs(lean(v)-Math.atan2(v.longitudinalSpeed*v.yawRate,g))>.01);
  const assets=createM4SpriteAssets();
  const sprite=createDynamicVehicleCourseSprite('bike',v,v.yaw,assets.bike,new HeightProfile(p.guide.length,[{s:0,y:0},{s:p.guide.length,y:0}]));
...
test('M9.28 lean line starts at ground anchor and continuously follows signed G beyond bitmap banks', async () => {
  for (const value of [-2.5,-1,0,1,2.5]) {
    const starts=[],ends=[],marks=[];
    const ctx={save(){},restore(){},beginPath(){},stroke(){},moveTo(x,y){starts.push([x,y]);},lineTo(x,y){ends.push([x,y]);},fillRect(...args){marks.push(args);}};
    drawVehicleLeanDebug(ctx,160,210,{lateralAcceleration:value*g});
    assert.deepEqual(starts,[[160,210],[160,210]]);
    const [x,y]=ends[0];assert.deepEqual(ends[0],ends[1]);
  ...
  assert.match(shell,/presentationFamily === 'BIKE'[\s\S]*drawVehicleLeanDebug\(ctx, camera.playerScreenX, playerScreenY, vehicle\)/);
