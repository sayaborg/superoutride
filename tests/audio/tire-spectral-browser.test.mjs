...
async function install(t) {
  const html = await readFile(new URL('../../tools/audio/tire-spectral-browser.html', import.meta.url), 'utf8');
  const source = html
    .match(/<script type="module">([\s\S]*?)<\/script>/)[1]
    .replace(/import\s+\{[^}]+\}\s+from\s+['"][^'"]+['"];?/g, '')
    .replaceAll('import.meta.url', JSON.stringify('https://example.test/tools/audio/tire-spectral-browser.html'));
