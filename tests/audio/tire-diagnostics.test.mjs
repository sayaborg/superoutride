...
import {
  ...
  runThrottleSweep,
  summarizeWindow,
} from '../../tools/physics/drift-control-probe.mjs';
...
function runReference(hz, direction, calibration = research) {
  const p = createFlatProbe({
    profile: withEngineCurveScale(VEHICLE_CATALOG[0].profile, 3),
    calibration,
...
for (const hz of [60, 120, 240])
  test(`no-TCS research reference at ${hz}Hz enters, traverses and exits in both directions`, () => {
    for (const sign of [-1, 1]) {
      ...
      assert.equal(t.rearLockTicks, 0);
      assert.equal(t.unsupportedTicks, 0);
      ...
      for (const [a, b, angle] of [
        ...
        const w = summarizeWindow(t, a, b);
        assert.ok(Math.abs(w.beta.mean + sign * angle) < 1);
        assert.ok(w.speed.min > 52 && w.speed.max < 59);
      }
      assert.ok(t.rows.filter((r) => r.t >= 30 && r.t <= 41).every((r) => Math.abs(r.beta) > 12));
      assert.ok(t.rows.filter((r) => r.t >= 42).every((r) => Math.abs(r.beta) < 1 && Math.abs(r.yawRate) < 0.05));
...
test('transient diagnostics are deterministic and preserve finite physical observations', () => {
  ...
  assert.equal(a.brakeTicks, 0);
  assert.ok(a.distance >= a.netDisplacement - 1e-6);
  assert.ok(
    a.rows.every((r) => [r.speed, r.beta, r.frontCapacityX, r.rearCapacityY, r.slipPower].every(Number.isFinite)),
  );
...
test('up/down sweep retains history and per-window ranges rather than claiming equilibria', () => {
  ...
  assert.deepEqual(
    s.samples.map((x) => x.leg),
    ['up', 'up', 'up', 'down', 'down', 'down'],
  );
  for (let i = 0; i < s.samples.length; i++) {
    const x = s.samples[i];
    assert.ok(x.window.beta.min <= x.window.beta.mean && x.window.beta.mean <= x.window.beta.max);
...
test('comparison forks copy normally reached state without mutable calibration sharing', () => {
  ...
  const before = p.vehicle.velocityX;
...
test('unprotected five-axis baseline remains finite for all nine profiles through input and recovery', () => {
  for (const { profile } of VEHICLE_CATALOG) {
    ...
    const v = p.vehicle,
      recovery = createRecoveryState(v);
    for (let tick = 0; tick < 360; tick++) {
      const input = { steering: tick < 120 ? 0 : tick < 240 ? 0.3 : -0.3, throttle: tick < 300, brake: tick >= 300 };
      updateArcadeVehicle({ guide: p.guide, height: p.height, surfaces: p.surface }, v, input, 1 / 60);
      assert.ok(
        [v.x, v.y, v.z, v.speed, v.yawRate, v.frontWheelOmega, v.rearWheelOmega].every(Number.isFinite),
        profile.id,
      );
      assert.equal(v.control.deliveredDriveTorque, v.powertrain.outputDriveTorque);
      updateRecovery({ guide: p.guide, height: p.height, surfaces: p.surface }, v, { state: recovery, dt: 1 / 60 });
    }
  }
});
test('diagnostics add no feedback, state seeding, gear lock or browser dependency', async () => {
  const src = await readFile(new URL('../../tools/physics/drift-control-probe.mjs', import.meta.url), 'utf8');
  assert.doesNotMatch(src, /\.gear\s*=|\.yaw\s*=|\.velocity[XYZ]\s*=|sCut|targetBeta/);
  assert.match(src, /curvature:/);
  assert.match(src, /forwardDisplacement/);
  assert.match(src, /runThrottleSweep/);
});
