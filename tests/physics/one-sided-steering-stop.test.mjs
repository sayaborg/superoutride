...
import { createFlatProbe } from '../../tools/physics/drift-control-probe.mjs';
...
test('ordinary high-speed brake replay removes the old rate-independent 20-degree input release', async () => {
  const { createTerrainProbe } = await import('../../tools/physics/torque-protection-terrain-probe.mjs');
  const { STEERING_STOP_CASES, runSteeringStopCase } =
    await import('../../tools/physics/steering-input-stop-probe.mjs');
