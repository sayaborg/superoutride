...
import { createFlatProbe, directInput, forkProbe, runProbe } from '../../tools/physics/drift-control-probe.mjs';
...
const tire = compileTireCharacteristics({ gripX: 2.5, peakSlipX: 0.08, gripY: 2.2, peakSlipY: 0.1, knee: 0.74 });
const wheel = (more = {}) => ({
  omegaPrevious: 30 / R,
  inertia: 3.4,
  rollingRadius: R,
  ...
  brakeTorque: 0,
  dt: 1 / 720,
  tire: car.profile.rearStation.tire,
  ...more,
});
...
test('policy is immutable composition data, with pitch protection only on the four bikes', () => {
  for (const [i, e] of VEHICLE_CATALOG.entries()) {
    ...
    const p = resolveTorqueProtectionPolicy(e.torqueProtection);
...
test('inverse torque equals the existing signed wheel equation and is strictly increasing', () => {
  for (let n = 0; n < 500; n++) {
    const i = wheel({
      ...
      longitudinalVelocity: 60 * random() - 30,
      lateralVelocity: 30 * random() - 15,
      normalLoad: 15000 * random(),
      gripFactor: 0.2 + random(),
...
test('TCS and ABS project simultaneous requested torques into feasible local bounds', () => {
  for (let n = 0; n < 2000; n++) {
    ...
    const i = wheel({
      ...
      omegaPrevious: (vx + s0 * ref) / R,
      lateralVelocity: 30 * random() - 15,
      gripFactor: m,
      normalLoad: 100 + 15000 * random(),
      driveTorque: 30000 * random(),
...
test('actual per-station torque telemetry conserves requested budget, not fixed delivered AWD split', () => {
  const e = VEHICLE_CATALOG[4],
    p = createFlatProbe({
      ...
      initialSpeed: 2,
      torqueProtection: e.torqueProtection,
    });
  let changed = false;
  for (let n = 0; n < 240; n++) {
    updateArcadeVehicle(
      ...
      p.vehicle,
      { steering: 0.3, throttle: true, brake: false },
      1 / 120,
    );
    const c = p.vehicle.control,
      req = p.vehicle.powertrain.outputDriveTorque;
    ...
    for (const side of ['Front', 'Rear'])
      assert.ok(c[side.toLowerCase() + 'DriveTorque'] <= c['requested' + side + 'DriveTorque'] + 1e-10);
    if (req > 0 && c.deliveredDriveTorque > 0 && Math.abs(c.frontDriveTorque / c.deliveredDriveTorque - 0.47) > 1e-5)
      changed = true;
  }
  assert.ok(changed);
});
  ...
  test(`all four bikes prevent repeated powered lift and braking overturn at ${hz} Hz`, () => {
    for (const e of VEHICLE_CATALOG.slice(5))
      for (const kind of ['drive', 'brake']) {
        ...
        assert.equal(x.rearLiftTime, 0, JSON.stringify(x));
        assert.equal(x.infeasibleTime, 0, JSON.stringify(x));
        if (kind === 'drive') assert.ok(x.finalSpeed > 20, e.profile.id);
        else {
          assert.ok(x.finalSpeed < 0.5);
          assert.ok(x.distance < 30);
          assert.ok(x.supportLimitedTime > 0);
        }
      }
  });
test('the unprotected bike failure remains reproducible and slip-only protection is not pitch protection', () => {
  ...
  const stillLifts = runProtectionProbe(slipOnly, { kind: 'brake' });
  assert.ok(stillLifts.rearLiftTime > 0);
  ...
  for (const e of VEHICLE_CATALOG) {
    ...
    for (let n = 0; n < 720; n++) {
      ...
      updateArcadeVehicle(
        { guide: p.guide, height: p.height, surfaces: p.surface },
        v,
        { steering: 0, throttle: t < 3, brake: t >= 3 },
        1 / 120,
      );
    ...
    assert.equal(v.torqueProtection, before);
...
test('neutral coasting is byte-identical physics, and support control does not glue an airborne body', () => {
  ...
  const a = runProbe(p, 2, () => directInput(0.65, 0)),
    b = runProbe(q, 2, () => directInput(0.65, 0));
  assert.deepEqual(a, b);
  const air = createFlatProbe({ profile: bike.profile, initialSpeed: 30, torqueProtection: TWO_WHEEL_TORQUE_POLICY });
  air.vehicle.y += 3;
  const y = air.vehicle.y;
  runProbe(air, 0.2, () => directInput(0, 0));
  assert.ok(air.vehicle.y < y);
...
test('compression barrier uses fresh geometry, velocity and the retained wrench rather than telemetry', () => {
  ...
  const q = forkProbe(p);
  q.vehicle.frontNormalLoad = 1e12;
  q.vehicle.rearNormalLoad = 0;
  q.vehicle.control.supportFeasible = false;
  runProbe(p, 0.2, () => directInput(0, 1));
  ...
  const a = supportCompressionMargin(v.profile, body, contact, wrench, 0.08);
  const rising = {
    ...contact,
...
test('protection adds no vehicle kind, target beta, body overwrite, or duplicated tire law', async () => {
  ...
  assert.match(s, /evaluateVehicleWrench/);
