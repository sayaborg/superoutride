...
} from '../../tools/physics/braking-yaw-probe.mjs';
import { forkProbe } from '../../tools/physics/drift-control-probe.mjs';
import {
  createTerrainProbe as createCurrentTerrainProbe,
  runTerrainProbe as runCurrentTerrainProbe,
  TERRAIN_CASES,
} from '../../tools/physics/torque-protection-terrain-probe.mjs';
...
test('replay fork preserves non-flat world readers and independent normally-reached state', () => {
  ...
  const before = brakingStateFingerprint(p.vehicle),
    q = forkProbe(p);
  ...
  updateArcadeVehicle({ guide: q.guide, height: q.height, surfaces: q.surface }, q.vehicle, input, 1 / 120);
...
test('read-only contact yaw attribution sums existing contact moment without feedback', () => {
  ...
  for (let n = 0; n < 180; n++)
    updateArcadeVehicle(
      ...
      v,
      { steering: 0.35, throttle: false, brake: n > 150 },
      1 / 120,
    );
...
test('VFR initial braking yaw growth precedes rack saturation and wheel lock', () => {
  ...
  assert.ok(early.rear.load < report.initial.rear.load * 0.4);
  assert.ok(early.rear.fy / early.rear.load > report.initial.rear.fy / report.initial.rear.load);
  assert.ok(Math.abs(early.rear.fy) < Math.abs(report.initial.rear.fy) * 0.4);
...
test('all-nine low-grip forks retain budgets and neutral release reduces peak beta', () => {
  for (const entry of VEHICLE_CATALOG) {
    const report = runBrakingComparison(entry, {
      grip: 0.25,
      actions: ['holdBrake25', 'holdBrake100', 'releaseBrake'],
    });
...
test('braking comparison is deterministic and action order does not mutate the parent', () => {
  ...
  const slow = runBrakingComparison(car, { speed: 0, seconds: 0.1, actions: ['holdCoast'] }).results[0];
...
test('input schedules and validation add no recovery, pose correction or closed-loop controller', async () => {
  ...
  assert.equal(brakingInput(0.1, 'counterPulseBrake', { direction: -1 }).steering, 0.35);
  for (const opts of [
    { hz: 90 },
    { seconds: 0 },
    { seconds: 0.0001 },
  ...
  const source = await readFile(new URL('../../tools/physics/braking-yaw-probe.mjs', import.meta.url), 'utf8');
  assert.doesNotMatch(
    source,
