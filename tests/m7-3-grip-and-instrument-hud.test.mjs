...
test('M8.0 SurfaceMap owns relative grip while tire profiles own reference friction', () => {
  ...
  assert.equal(
    HONDA_VFR750R_VEHICLE_PROFILE.frontStation.tire.muY,
    FERRARI_TESTAROSSA_VEHICLE_PROFILE.frontStation.tire.muY,
  );
...
test('M9.9 axle-neutral tire still keeps useful linear capacity inside the shared one-k radial knee', () => {
  ...
  const frontNormal =
    (FERRARI_TESTAROSSA_VEHICLE_PROFILE.mass * 9.80665 * FERRARI_TESTAROSSA_VEHICLE_PROFILE.rearAxle) /
  ...
  assert.ok(
    Math.abs(
      capacity -
        FERRARI_TESTAROSSA_VEHICLE_PROFILE.frontStation.tire.rhoKnee *
          FERRARI_TESTAROSSA_VEHICLE_PROFILE.frontStation.tire.muY *
          frontNormal,
    ) < 1e-9,
  );
...
test('M9.9 common preset-1 tire keeps one broad symmetric transition shoulder', () => {
  const pureLateralAngles = (normalizedStiffness) => ({
    ...
    plateauStart:
      (Math.atan(
        ((2 - FERRARI_TESTAROSSA_VEHICLE_PROFILE.frontStation.tire.rhoKnee) *
          FERRARI_TESTAROSSA_VEHICLE_PROFILE.frontStation.tire.muY) /
          normalizedStiffness,
      ) *
        180) /
