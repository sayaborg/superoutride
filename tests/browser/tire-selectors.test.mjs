...
import { createFlatProbe, directInput, runProbe } from '../../tools/physics/drift-control-probe.mjs';
...
test('every endpoint combination is admissible, without old S<=G filtering', () => {
  for (let mask = 0; mask < 32; mask++) {
    ...
    for (let i = 0; i < axes.length; i++) {
      const a = axes[i];
      c = pair(compile(browserTireCalibrationForAxis(a.id, (mask & (1 << i) ? a.max : a.min) / 100, c)));
    }
...
test('explicit minus/plus and one-key forward cycle share the same five linked settings', () => {
  ...
  for (let i = 0; i < axes.length; i++) {
    const old = read(v.tireFrictionCalibration.front),
      a = axes[i],
      group = host.children[i];
...
test('live selection changes only calibration and remains atomic on invalid request', () => {
  ...
  delete before.tireFrictionCalibration;
  delete after.tireFrictionCalibration;
...
test('recovery and all-nine vehicle replacement preserve selections without sharing mutable state', () => {
  ...
  for (const { profile } of VEHICLE_CATALOG) {
    const next = createArcadeVehicle(
      profile,
      { guide: p.guide, height: p.height, surfaces: p.surface },
      { s: 10000, l: 0, initialSpeed: 15, steeringCalibration: {}, tireFrictionCalibration: c },
    );
    assert.deepEqual(next.tireFrictionCalibration, c);
    assert.notEqual(next.tireFrictionCalibration, c);
    set(next, seed);
    assert.equal(v.tireFrictionCalibration, c);
  }
});
