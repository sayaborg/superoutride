...
for (const [root, mode] of [
  ['main-linear', 'linear'], ['main', 'branching'],
  ['main-circuit', 'circuit'], ['main-circuit', 'fisco'],
]) {
  test(`${mode}: vehicle replacement renders before the next physics tick`, async t => {
    ...
    browser.frame();
    const select = name => browser.elements.get('vehicle-selector-buttons')
      .children.find(element => element.textContent === name).emit('click');
