...
const surfaces = new SurfaceMap(guide.length, [
  { sStart: 0, name: 'wheel-lift fixture', bands: [{ lMin: -1000, lMax: 1000, type: 'ASPHALT' }] },
]);
...
test('M9.18 wheelie and stoppie single-contact poses retain ordinary forces without recovery', () => {
  for (const degrees of [-80, -45, -15, 15, 45, 80]) {
    ...
    v.y = loadedStation.freeReachDown * Math.cos(v.pitch) - loadedStation.forwardOffset * Math.sin(v.pitch) - 0.03;
    const front = observe(v, profile.frontStation),
      rear = observe(v, profile.rearStation);
    const loaded = degrees > 0 ? rear : front,
      lifted = degrees > 0 ? front : rear;
    ...
    const state = createRecoveryState(v);
    assert.equal(updateRecovery({ guide, height, surfaces }, v, { state, dt: 1 / 60 }), null);
  ...
  for (const target of [null, { s: 750, l: 1 }]) {
    const v = createArcadeVehicle(
      ...
      { guide, height, surfaces },
      {
        ...
        l: 0,
        initialSpeed: 25,
        steeringCalibration: { maxRoadWheelSteer: 60 * DEG, steeringOffsetMax: 12 * DEG },
        tireFrictionCalibration: createArcadeTireFrictionCalibration(
          compileTireCharacteristics({ gripX: 2.7, gripY: 2.7, peakSlipX: 0.25, peakSlipY: 0.25, knee: 0.74 }),
        ),
      },
    );
    ...
    assert.equal(
      updateRecovery({ guide, height, surfaces }, v, {
        state,
        dt: 1 / 60,
        target,
      }),
...
test('M9.18 overturned criterion is relative to surface normal rather than a world-pitch clamp', () => {
  ...
  const uphill = new HeightProfile(guide.length, [
    { s: 0, y: 0 },
    { s: guide.length, y: slope * guide.length },
  ]);
  const v = createArcadeVehicle(profile, { guide, height: uphill, surfaces }, { s: 800, l: 0, initialSpeed: 15 });
  const state = createRecoveryState(v);
...
test('M9.18 upright suspension travel guard is retained rather than clipped or disabled', () => {
  ...
  assert.throws(
    () => observe(v, profile.frontStation),
...
test('M9.18 VFR loop-out remains possible but ordinary recovery prevents inverted driving at refined steps', () => {
  ...
  for (const dt of [1 / 60, 1 / 120, 1 / 240]) {
    for (const kind of ['reversal', 'tsukuba']) {
      ...
      const v = createArcadeVehicle(
        historicalProfile,
        { guide: g, height: h, surfaces: s },
        {
          s: live ? 45 : 800,
          l: 0,
          initialSpeed: live ? 15 : 25,
          steeringCalibration: live
            ? {}
            : {
