...
function fixture(speed=30,beta=0,pitch=0){
 ...
 return {body,c:{...c,forceTransmitting:true,tireFrameValid:true,reachVelocity:{x:speed*Math.sin(beta),y:0,z:speed*Math.cos(beta)}}};
}
...
function excess(body,c,angle){
 const f=reorientContactObservation(c,body,angle),n=c.surface.normal;
 const h={x:body.forward.x*Math.cos(angle)+body.right.x*Math.sin(angle),y:body.forward.y*Math.cos(angle)+body.right.y*Math.sin(angle),z:body.forward.z*Math.cos(angle)+body.right.z*Math.sin(angle)};
 const normal=h.x*n.x+h.y*n.y+h.z*n.z;
 const norm2=h.x*h.x+h.y*h.y+h.z*h.z-normal*normal;
 const s=c.surface.material.gripFactor*.08;
 return norm2*(f.lateralVelocity**2-s*s*(f.longitudinalVelocity**2+c.profile.tire.lowSpeedRegularization**2));
}

test('M9.27 ordinary stop retains small input, approaches onset conservatively and passes flat standstill',()=>{
 for(const speed of [0,.001,1,30])for(const sign of [-1,1]){
  const {body,c}=fixture(speed),d=sign*20*rad,e=limit(0,d,body,c,tire);
  assert.equal(limit(0,0,body,c,tire),0);
  if(speed<=.001)assert.equal(e,d);
  else{assert.ok(Math.abs(e)<Math.abs(d));assert.ok(excess(body,c,e)<=1e-10);}
 }
 const {body,c}=fixture();assert.equal(limit(0,.01,body,c,tire),.01);
...
test('M9.27 outside baseline stops farther input, passes partial correction, and stops before opposite excess',()=>{
 for(const sign of [-1,1]){
  ...
  assert.ok(Math.abs(b+e)<Math.atan(.08)+1e-4);
 }
});
test('M9.27 former 0-to-20-degree tangent release is continuous and does not inflate onset',()=>{
 ...
 for(const beta of [-129.99,-129.9999,-130,-130.0001,-130.01]){
  ...
  const e=limit(-40*rad,20*rad,body,c,tire);values.push(e);
  assert.ok(Math.abs(e)<.011*rad);
 }
 assert.ok(Math.max(...values)-Math.min(...values)<.011*rad);
});
test('M9.27 exact physical excess respects the quadratic certificate through signed tilted correction paths',()=>{
 let seed=20260910;const rng=()=>((seed=(Math.imul(seed,1664525)+1013904223)>>>0)/2**32);
 let inside=0,outside=0,noZero=0;
 for(let i=0;i<1500;i++){
  const {body,c:base}=fixture(.1+rng()*80,(rng()-.5)*2*Math.PI,(rng()-.5)*.8);
  const nx=(rng()-.5)*.5,nz=(rng()-.5)*.5,z=Math.hypot(nx,1,nz);
  const c={...base,surface:{...base.surface,normal:{x:nx/z,y:1/z,z:nz/z},material:{...base.surface.material,gripFactor:.25+rng()*2}}};
  const b=(rng()-.5)*80*rad,d=(rng()-.5)*40*rad,e=limit(b,d,body,c,tire);
  const q=x=>excess(body,c,x),v=q(b),other=q(b+Math.PI/2);
  const cc=(v-other)/2,cs=(q(b+Math.PI/4)-q(b-Math.PI/4))/2,R=Math.hypot(cc,cs),g=2*cs;
  const Q=x=>v+g*x+2*R*x*x;
  const minQ=v-g*g/(8*R);
  if(v<=0)inside++;else outside++;
  if(minQ>0){noZero++;assert.ok(Math.abs(e)<=Math.abs(-g/(4*R))+1e-8);}
  ...
  for(let k=-19;k<=20;k++){
   ...
   assert.ok(next>=previous-1e-12&&next-previous<=rad+1e-12);previous=next;
  }
 }
...
test('M9.27 absent physical contact retains existing bypass, without a new surface or force observation',()=>{
 ...
 for(const x of [{...c,forceTransmitting:false},{...c,tireFrameValid:false},{...c,surface:{...c.surface,material:{...c.surface.material,gripFactor:0}}},{...c,surface:{...c.surface,normal:body.right}}])assert.equal(limit(.1,.2,body,x,tire),.2);
});
test('M9.27 one fixed onset and one quadratic replace baseline inflation and angular root selection',async()=>{
 ...
 assert.doesNotMatch(src,/baseSlip|sAllowed|Math\.acos|Math\.atan2|\bfor\s*\(|\bwhile\s*\(|solveWheelOmega|evaluateTireForce|\.omega|yawRate|speedTable/);
...
test('M9.27 ordinary high-speed brake replay removes the old rate-independent 20-degree input release',async()=>{
 ...
 const scenario=STEERING_STOP_CASES.find(x=>x.name==='fullBrakeHold'),steps=[];
 for(const hz of [60,120,240]){
  const parent=createTerrainProbe(VEHICLE_CATALOG[0],{speed:60});setArcadeVehicleSteeringOffsetMax(parent.vehicle,20*rad);
  const result=runSteeringStopCase(parent,scenario,{hz});
  assert.ok(result.cutSeconds>0);assert.equal(result.unsupportedSeconds,0);steps.push(result.maxInputStepDeg);
 }
 assert.ok(steps[1]<steps[0]*.6&&steps[2]<steps[1]*.6);
