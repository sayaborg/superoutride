  ...
  assert.throws(
    () =>
      compileRasterSuccessorChain({
        ...
        createRuntime: () => fakeRuntime(initial, 'PKG_GOAL'),
      }),
...
test('runtime helper is renderer, route-DAG and vehicle-physics independent while delegates deep LEFT construction', async () => {
  const [compiler, courseAuthoring] = await Promise.all([
  ...
  assert.match(courseAuthoring, /compileRasterSuccessorChain/);
  assert.match(courseAuthoring, /repackageGuideChartRuntime/);
  assert.doesNotMatch(courseAuthoring, /createRasterStageSuccessor/);
});
