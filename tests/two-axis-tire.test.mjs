...
function legacyH(r, a) {
  ...
  const w = 2 - 2 * a,
    t = (r - a) / w;
  return (2 * t ** 3 - 3 * t ** 2 + 1) * a + (t ** 3 - 2 * t ** 2 + t) * w + (-2 * t ** 3 + 3 * t ** 2);
}
let n = 123456789;
const random = () => {
...
for (const field of Object.keys(seed))
  test(`M9.20 ${field} rejects bad compilation atomically`, () => {
    ...
    for (const bad of invalid) {
      assert.throws(() => set(owner, { ...seed, [field]: bad }), RangeError);
...
test('M9.20 linked editing has one immutable atomic pair, with independent owner lifecycle', () => {
  ...
  set(a, { ...seed, gripX: 3 });
  assert.notEqual(a.tireFrictionCalibration, old);
  assert.equal(a.tireFrictionCalibration.front, a.tireFrictionCalibration.rear);
  assert.ok(Object.isFrozen(a.tireFrictionCalibration.front));
  assert.equal(b.tireFrictionCalibration.front.muX, 2.5);
  assert.throws(() => {
    ...
    for (const a of [0.1, 0.74, 0.95])
      for (const N of [1, 100, 10000])
        for (const m of [0.3, 1, 1.6]) {
          const c = compile({ ...seed, knee: a }),
            P = seed[axis === 'X' ? 'peakSlipX' : 'peakSlipY'],
...
test('M9.20 pure/combined forces remain bounded, dissipative, symmetric and load-homogeneous', () => {
  for (let i = 0; i < 20000; i++) {
    const c = compile({
      ...
      gripY: 0.5 + 3.5 * random(),
      peakSlipX: 0.01 + 0.59 * random(),
      peakSlipY: 0.01 + 0.59 * random(),
      knee: 0.1 + 0.85 * random(),
    });
    const sx = 4 * random() - 2,
      sy = 4 * random() - 2,
      N = 0.01 + 15000 * random(),
      m = 0.1 + 1.5 * random();
    const f = at(c, sx, sy, N, m),
      f2 = at(c, sx, sy, 2 * N, m),
  ...
  for (const a of [0.1, 0.74, 0.95])
    for (const py of [0.01, 0.1, 0.6])
      for (const sy of [-1, -0.1, 0, 0.1, 1]) {
        const c = compile({ ...seed, knee: a, peakSlipY: py });
        let previous = -Infinity,
          lastFy = Infinity;
        for (let i = 0; i <= 400; i++) {
          const sx = (i - 200) / 100,
            f = at(c, sx, sy);
          assert.ok(f.fx >= previous - 1e-8);
          previous = f.fx;
          if (i >= 200) {
            assert.ok(Math.abs(f.fy) <= lastFy + 1e-8);
            lastFy = Math.abs(f.fy);
          }
        }
      }
});
test('M9.20 absent contact and zero-grip road release exact force without a friction/load floor', () => {
  ...
  for (const [N, m] of [
    ...
    const f = at(c, 3, -2, N, m);
    assert.equal(f.fx, 0);
    assert.equal(f.fy, 0);
    assert.equal(f.capacityX, 0);
    assert.equal(f.capacityY, 0);
  }
