...
test('M5 player and camera share render height space while suspension displacement remains visible', () => {
  ...
  const camera = updateCamera(createCameraRig(), parent.guide, height, car, cameraProfile, 1 / 60);
  ...
  const oldPlayer = pseudoProject({ x: car.x, y: car.presentationY, z: car.z, s: car.course.s }, camera);
  const oldRoad = pseudoProject({ x: car.x, y: renderRoadY, z: car.z, s: car.course.s }, camera);
  ...
  const renderRoad = pseudoProject({ x: car.x, y: renderRoadY, z: car.z, s: car.course.s }, renderCamera);
