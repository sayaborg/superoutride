...
const surfaceSection = () => ({
  sStart: 0, name: 'road', bands: [{ lMin: -4, lMax: 4, type: 'ASPHALT' }],
});

test('nonfinite endpoint authoring is rejected before normalization can hide it', () => {
  for (const value of [NaN, Infinity, -Infinity]) {
    assert.throws(() => new HeightProfile(100, [{ s: value, y: 0 }, { s: 100, y: 0 }]));
    assert.throws(() => new HeightProfile(100, [{ s: 0, y: 0 }, { s: value, y: 0 }]));
...
test('source profiles own immutable geometry, paint and material copies', () => {
  const nodes = [{ s: 0, y: 0 }, { s: 100, y: 10 }];
  ...
  assert.equal(surface.sample(50, 0).type, 'ASPHALT');
  assert.throws(() => { height.nodes[1].y = 999; }, TypeError);
  assert.throws(() => { visual.sample(50).groundBaseLeft.color = 0; }, TypeError);
  assert.throws(() => { logical.sample(50).left = 'GRASS'; }, TypeError);
  assert.throws(() => { surface.sectionAt(50).bands[0].lMax = -3; }, TypeError);
  assert.throws(() => { surface.sample(50, 0).material.gripFactor = 999; }, TypeError);
});
