...
function fixture(entry = VEHICLE_CATALOG[0], profile = entry.profile) {
  ...
  const surface = new SurfaceMap(guide.length, [
    { sStart: 0, name: 'HUD observation', bands: [{ lMin: -500, lMax: 500, type: 'ASPHALT' }] },
  ]);
...
function hud(v, input = neutral) {
  return createVehicleDebugHudModel('linear', input, v);
}
function drive(v, a, total, frontDelivered, rearDelivered) {
  const f = v.profile.frontDriveTorqueFraction;
  Object.assign(v.control, {
    throttleActuator: a,
    requestedFrontDriveTorque: total * f,
    requestedRearDriveTorque: total * (1 - f),
    frontDriveTorque: frontDelivered,
    rearDriveTorque: rearDelivered,
  });
}
function brake(v, b, frontDelivered, rearDelivered) {
  Object.assign(v.control, {
    brakeActuator: b,
    requestedFrontBrakeTorque: b * v.profile.frontStation.maxBrakeTorque,
    requestedRearBrakeTorque: b * v.profile.rearStation.maxBrakeTorque,
    frontBrakeTorque: frontDelivered,
    rearBrakeTorque: rearDelivered,
  });
}
function canvasRecorder() {
  ...
  const ctx = {
    ...
    strokeText() {},
    fillText(text, x, y) {
      texts.push({ text, x, y, color: this.fillStyle });
    },
  };
...
test('input meters preserve exact analog requests, digital shorthand and exclusivity', () => {
  ...
  for (const input of [
    { throttle: 0.2, brake: 0.2 },
    { throttle: NaN, brake: 0 },
    { throttle: 0, brake: 1.01 },
  ])
...
test('AWD 45:55 shares a full-throttle scale and does not renormalize delivered total', () => {
  const entry = VEHICLE_CATALOG[4],
    profile = compileArcadeVehicleProfile({
      ...LANCIA_DELTA_HF_INTEGRALE_VEHICLE_AUTHORING,
      frontDriveTorqueFraction: 0.45,
    });
  const { vehicle: v } = fixture(entry, profile);
  drive(v, 1, 1000, 450, 550);
  let h = hud(v, { ...neutral, throttle: 1 });
  close(h.frontDrive.requested, 0.45);
  close(h.rearDrive.requested, 0.55);
  close(h.frontDrive.delivered, 0.45);
  close(h.rearDrive.delivered, 0.55);
  close(h.frontDrive.limit, 0.45);
  close(h.rearDrive.limit, 0.55);
  drive(v, 0.5, 500, 225, 275);
  h = hud(v, { ...neutral, throttle: 0.5 });
  close(h.frontDrive.delivered, 0.225);
  close(h.rearDrive.delivered, 0.275);
  drive(v, 1, 1000, 225, 550);
  h = hud(v, { ...neutral, throttle: 1 });
  close(h.frontDrive.delivered, 0.225);
  close(h.rearDrive.delivered, 0.55);
  close(h.frontDrive.requested - h.frontDrive.delivered, 0.225);
  close(h.frontDrive.delivered + h.rearDrive.delivered, 0.775);
});
...
for (const index of [0, 3, 4, 5])
  test(`${VEHICLE_CATALOG[index].profile.id} drive bars derive authored split without identity branches`, () => {
    ...
    const h = hud(v);
    close(h.frontDrive.delivered, 0.4 * f);
    close(h.rearDrive.delivered, 0.4 * (1 - f));
...
test('brake percentages use fixed sum of front/rear torque capacity, including 70:30 cuts', () => {
  ...
  brake(v, 1, 500, 100);
  let h = hud(v, { ...neutral, brake: 1 });
  close(h.frontBrake.limit, 0.7);
  close(h.rearBrake.limit, 0.3);
  close(h.frontBrake.requested, 0.7);
  close(h.rearBrake.requested, 0.3);
  close(h.frontBrake.delivered, 0.5);
  close(h.rearBrake.delivered, 0.1);
  close(h.frontBrake.requested - h.frontBrake.delivered, 0.2);
  close(h.rearBrake.requested - h.rearBrake.delivered, 0.2);
  brake(v, 0.5, 350, 150);
  h = hud(v, { ...neutral, brake: 0.5 });
  close(h.frontBrake.delivered, 0.35);
...
test('lag and residual-pedal overlap are distinct from protection cuts', () => {
  ...
  let h = hud(v, { ...neutral, throttle: 1 });
  close(h.requestedThrottle, 1);
  close(h.rearDrive.delivered, 0.2);
  close(h.rearDrive.requested - h.rearDrive.delivered, 0);
  brake(v, 0.1, v.profile.frontStation.maxBrakeTorque * 0.1, v.profile.rearStation.maxBrakeTorque * 0.1);
  h = hud(v, { ...neutral, brake: 1 });
  assert.equal(h.requestedThrottle, 0);
  close(h.rearDrive.delivered, 0.2);
  close(h.frontBrake.delivered + h.rearBrake.delivered, 0.1);
  const c = canvasRecorder();
  drawVehicleControlGraphics(c.ctx, h, 3, 79);
...
test('zero requests/full engine cut and zero brake capacity have finite empty bars', () => {
  ...
  assert.equal(h.requestedThrottle, 1);
  assert.equal(h.frontDrive.requested + h.rearDrive.requested, 0);
  ...
  const p = compileArcadeVehicleProfile({
    ...FERRARI_TESTAROSSA_VEHICLE_AUTHORING,
    frontBrakeTorqueMax: 0,
    rearBrakeTorqueMax: 0,
  });
  const z = hud(fixture(VEHICLE_CATALOG[0], p).vehicle, { ...neutral, brake: 1 });
  assert.deepEqual(z.frontBrake, { requested: 0, delivered: 0, limit: 0 });
...
test('normalization is from one torque sample, independent of post-sample gear/RPM and load caches', () => {
  ...
  brake(v, 0.2, 500, 100);
  const a = hud(v);
  v.powertrain.engineRpm = 6800;
  v.powertrain.gear = 5;
  v.frontNormalLoad = 0;
  v.rearNormalLoad = 123456;
  const b = hud(v);
...
test('red paints exactly delivered-to-request interval and leaves unused capacity empty', () => {
  ...
  brake(v, 1, 500, 100);
  const c = canvasRecorder();
  drawVehicleControlGraphics(c.ctx, hud(v, { ...neutral, brake: 0.5 }), 3, 79);
  const cuts = c.rects.filter((r) => r.color === HUD_PROTECTION_CUT_COLOR && r.y > 90);
  assert.equal(cuts.length, 2);
  close(cuts[0].x, 3 + 170 + 2 + 0.5 * 54);
  close(cuts[0].w, 0.2 * 54);
  assert.equal(cuts[0].y, 79 + 22 + 2);
  close(cuts[1].x, 3 + 170 + 2 + 0.1 * 54);
  close(cuts[1].w, 0.2 * 54);
  assert.equal(cuts[1].y, 79 + 36 + 2);
  const delivered = c.rects.filter((r) => r.color === HUD_DELIVERED_COLOR && r.x === 175 && r.w > 0);
  close(delivered[0].w, 0.5 * 54);
  close(delivered[1].w, 0.1 * 54);
...
test('input fill is proportional, not a positive-value ON/OFF indicator', () => {
  ...
  for (const [axis, color] of [
    ...
    ['brake', HUD_INPUT_BRAKE_COLOR],
  ]) {
    const c = canvasRecorder();
    drawVehicleControlGraphics(c.ctx, hud(v, { ...neutral, [axis]: 0.25 }), 3, 79);
    const active = c.rects.filter((r) => r.color === color && r.w > 0);
    assert.equal(active.length, 1);
    close(active[0].w, 0.25 * 54);
  }
});
...
test('six pedal meters plus retained steering fit 320x240 without a background panel', () => {
  ...
  brake(v, 1, 500, 100);
  const c = canvasRecorder();
  drawVehicleDebugHud(c.ctx, 'linear', { ...neutral, brake: 1 }, v);
  assert.equal(c.boxes.length, 10);
  assert.equal(c.texts.filter((t) => t.text === 'F').length, 2);
...
for (const entry of VEHICLE_CATALOG)
  test(`${entry.profile.id} live protection telemetry remains read-only and budget-consistent`, () => {
    ...
    for (let tick = 0; tick < 180; tick++) {
      ...
      updateArcadeVehicle({ guide: p.guide, height: p.height, surfaces: p.surface }, v, input, 1 / 60);
      const before = JSON.stringify(v),
        h = hud(v, input),
        c = canvasRecorder();
      drawVehicleDebugHud(c.ctx, 'linear', input, v);
      assert.equal(JSON.stringify(v), before);
      for (const key of ['frontDrive', 'rearDrive', 'frontBrake', 'rearBrake']) {
        const m = h[key];
        assert.ok(
          Number.isFinite(m.requested) &&
            0 <= m.delivered &&
            m.delivered <= m.requested &&
            m.requested <= m.limit &&
            m.limit <= 1,
          key,
        );
      ...
      if (d > 0)
        close(
          h.frontDrive.delivered + h.rearDrive.delivered,
          (v.control.throttleActuator * (v.control.frontDriveTorque + v.control.rearDriveTorque)) / d,
        );
    }
  });
...
test('HUD removes ambiguous actuator-only pedal fields and adds no physical control path', async () => {
  ...
  assert.doesNotMatch(
    s,
    /updateArcadeVehicle|updateAutomaticPowertrain|solveWheelOmega|solveProtectedWheelPair|globalAlpha|setTimeout/,
  );
