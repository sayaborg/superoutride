...
test('compiled actuators and powertrain own their validated nested data', () => {
  ...
  for (const object of [
    profile.actuator,
    profile.actuator.throttle,
    profile.actuator.brake,
    profile.actuator.steering,
    profile.powertrain,
    profile.powertrain.gearRatios,
    profile.powertrain.torqueCurve,
    ...profile.powertrain.torqueCurve,
  ]) {
...
test('mechanics owns no production records, membership list or upward content import', async () => {
  ...
  for (const name of await readdir(new URL('../src/physics/', import.meta.url))) {
    ...
    assert.doesNotMatch(
      source,
      /(?:from|import\s*\()\s*['"][^'"]*(?:\/vehicle\/|production-vehicle-profiles|vehicle-catalog)/,
      name,
    );
  }
...
test('compiled profiles expose one resolved station authority, not authored wheel/tire/suspension copies', () => {
  const authored = {
    ...
    rearWheelRadius: 0.41,
    frontWheelInertia: 3,
    rearWheelInertia: 4,
    frontBrakeTorqueMax: 111,
    rearBrakeTorqueMax: 222,
  ...
  for (const key of [
    ...
    'frontTire',
    'rearTire',
    'lowSpeedRegularization',
    'unexpectedAuthoringField',
  ]) {
