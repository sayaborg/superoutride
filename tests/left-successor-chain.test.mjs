...
test('LEFT third-stage path remains present as later milestones deepen the RIGHT route too', () => {
  const live = setup();
  assert.deepEqual(
...
test('promotes the old LEFT goal package geometry into STAGE_3_L and gives GOAL_L a new chart', () => {
  const live = setup();
...
test('third LEFT transition owns a physical handoff to the new GOAL_L chart and content binding', () => {
  const live = setup();
...
test('leaves renderer and browser loop route-agnostic as both branches deepen', async () => {
