...
test('browser authority exposes only M D and T steering selectors', () => {
  assert.deepEqual(BROWSER_STEERING_OFFSETS.map(({ degrees }) => degrees), [9, 9.5, 11, 12.5, 14]);
  assert.deepEqual(BROWSER_MAX_ROAD_WHEEL_STEERS.map(({ degrees }) => degrees), [37, 41, 45, 49, 53]);
  assert.deepEqual(BROWSER_STEERING_RESPONSES.map(({ traversalSeconds }) => traversalSeconds), [0.25, 0.375, 0.5, 0.625]);
...
test('every M x D choice derives A=M-D and preserves exact driver reserve inside A', () => {
  ...
  assert.ok(Math.abs(minimumAutomaticDegrees - 23) < 1e-12);
});
...
test('all nine profiles remain finite at the smallest M largest D and slowest T selector corner', () => {
  for (const { profile } of VEHICLE_CATALOG) {
    ...
    assert.ok(Math.abs(vehicle.frontSteerAngle) <= 37 * DEG + 1e-12, profile.id);
  }
});
