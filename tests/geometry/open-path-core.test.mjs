...
test('RasterPath endpoints have no synthetic closure turn or miter', () => {
  ...
  const start = rasterPathToWorld(raster, 0, 3, createPlanarCoordinateSample());
  const end = rasterPathToWorld(raster, raster.length, 3, createPlanarCoordinateSample());
  assert.notDeepEqual({ x: start.x, z: start.z }, { x: end.x, z: end.z });
});

test('open RasterPath sampling never wraps out-of-range chainage', () => {
  const { raster } = createOpenFixture();
  near(sampleRasterPath(raster, 0, createPlanarCoordinateSample()).s, 0, 1e-8);
  near(sampleRasterPath(raster, raster.length, createPlanarCoordinateSample()).s, raster.length, 1e-8);
  assert.throws(() => sampleRasterPath(raster, -1, createPlanarCoordinateSample()), /outside/);
  assert.throws(() => sampleRasterPath(raster, raster.length + 1, createPlanarCoordinateSample()), /outside/);
});
...
test('GuidePath has no endpoint wrap fillet and does not sample cyclically', () => {
  ...
  near(sampleGuidePath(guide, 0, createPlanarCoordinateSample()).s, 0, 1e-8);
  near(sampleGuidePath(guide, guide.length, createPlanarCoordinateSample()).s, guide.length, 1e-8);
  assert.throws(() => sampleGuidePath(guide, -1, createPlanarCoordinateSample()), /outside/);
  assert.throws(() => sampleGuidePath(guide, guide.length + 1, createPlanarCoordinateSample()), /outside/);
});

test('local world-to-Guide search clips indices instead of wrapping endpoints', () => {
  const { guide } = createOpenFixture();
  const target = guidePathToWorld(guide, guide.length - 2, 0, createPlanarCoordinateSample());
  const fromStartWindow = locateWorldOnGuideLocal(
    guide,
    target,
    0,
    1,
...
test('forward terrain interval clips at the open path endpoint instead of wrapping', () => {
  ...
  const interval = computeForwardVisibleInterval(
    guide,
    sampleGuidePath(guide, cameraS, createPlanarCoordinateSample()).heading,
    cameraS,
    2.5,
    150,
  );
