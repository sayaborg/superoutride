...
test('tire calibration exposes the ellipse axes with no hidden exponent', async () => {
  assert.deepEqual(
    BROWSER_TIRE_AXES.map((a) => a.id),
    ['GX', 'PX', 'GY', 'PY', 'KNEE'],
  );
  assert.ok(BROWSER_TIRE_AXES.every((a) => a.code !== 'KeyB'));
  assert.deepEqual(Object.keys(compileTireCharacteristics(DEFAULT_BROWSER_TIRE_CHARACTERISTICS)).sort(), [
    'kX',
    'kY',
    'muX',
    'muY',
  ...
  async function scan(dir) {
    for (const e of await readdir(dir, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) await scan(path);
        ...
        assert.doesNotMatch(await readFile(path, 'utf8'), /combinedSlipExponent|combinedSlipNorm/, path.pathname);
    }
  }
  await scan(new URL('../src/', import.meta.url));
  await scan(new URL('../tools/', import.meta.url));
});
