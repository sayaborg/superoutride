...
test('all nine stock profiles launch through ordinary wheel/contact dynamics and reconstruct unscaled powertrain in recovery', () => {
  for (const { profile } of VEHICLE_CATALOG) {
    ...
    for (let tick = 0; tick < 120; tick++)
      updateArcadeVehicle({ guide, height, surfaces: surface }, v, { steering: 0, throttle: true, brake: false }, dt);
...
function huntingProbe() {
  ...
  for (let tick = 0; tick < 1200; tick++) {
    updateArcadeVehicle({ guide, height, surfaces: surface }, v, input, dt);
