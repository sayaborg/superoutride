...
test('invalid authored GroundBase colors fail before numeric coercion in either source', () => {
  for (const color of [NaN, Infinity, -1, 0.5, 0x100000000]) {
    const section = { sStart: 0, name: 'test', groundBaseLeft: { kind: 'color', color },
      groundBaseRight: { kind: 'transparent' }, groundMapLeft: 'GRASS', groundMapRight: 'GRASS',
...
test('compiled route observations cannot mutate validated graph topology', () => {
  const route = compileRouteDag('a', [{ id: 'a', kind: 'STAGE' }, { id: 'b', kind: 'TERMINAL' }],
  ...
  assert.throws(() => { result.acceptedChoice.toStageId = 'a'; }, TypeError);
  assert.throws(() => { route.stages[1].kind = 'STAGE'; }, TypeError);
