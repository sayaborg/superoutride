...
test('browser course selector maps 1/2/3/4 and URL modes from one authority', () => {
  assert.deepEqual(
    BROWSER_COURSE_MODES.map(({ digitCode, query, routeKind, entryName }) => ({ digitCode, query, routeKind, entryName })),
    [
...
test('browser vehicle selector derives all nine exact keys and profiles from the product catalog', () => {
  assert.deepEqual(BROWSER_VEHICLE_PROFILES.map(({ code, profile }) => [code, profile.id]), [
    ['KeyQ', 'TESTAROSSA'], ['KeyW', '911_TURBO_3_3'], ['KeyE', 'CORVETTE_C4'],
    ['KeyR', 'GOLF_GTI_16V'], ['KeyA', 'DELTA_HF_INTEGRALE'], ['KeyS', 'VFR750R'],
    ['KeyD', 'R80_GS_PARIS_DAKAR'], ['KeyF', 'FXRT_SPORT_GLIDE'], ['KeyV', 'PX200E_ARCOBALENO'],
  ]);
  for (const catalogEntry of VEHICLE_CATALOG) assert.equal(browserVehicleProfileForKey(catalogEntry.keyCode), catalogEntry.profile);
...
test('catalog profiles share only the M9.8 normalized tire law and retain distinct mechanics', () => {
  ...
  assert.equal(VOLKSWAGEN_GOLF_GTI_16V_VEHICLE_PROFILE.frontAxle + VOLKSWAGEN_GOLF_GTI_16V_VEHICLE_PROFILE.rearAxle,
...
test('LINEAR debug course is one finite ordinary open 8 km highway and renders normally', () => {
  ...
  const camera = updateM5Camera(createM5CameraRig(), runtime.guide, runtime.heightProfile, car, CAMERA_PROFILE, DT);
  const result = renderM5Driving(new SoftwareSurface(320,240,new Uint32Array(320*240)),createM3FarBackground(),runtime.guide,camera,car,runtime.terrainProfile,runtime.groundProfile,[],createM4SpriteAssets(),'car');
  assert.ok(result.terrainLineCount > 0); assert.equal(camera.playerScreenX,160);
});
...
test('all nine vehicle profiles integrate on the finite LINEAR course with permitted wheel lift and recovery', () => {
  ...
  for(const {profile} of VEHICLE_CATALOG){
    ...
    for(let tick=0;tick<600;tick++){
      ...
      for(const value of [vehicle.x,vehicle.y,vehicle.z,vehicle.speed,vehicle.pitch,vehicle.pitchRate])assert.ok(Number.isFinite(value),profile.id);
...
test('G sensor draws only one cross and one dot in the felt inertial-load direction', () => {
  const point=gSensorPoint({longitudinalG:1,lateralG:.5},100,80,20); assert.deepEqual(point,{x:95,y:90});
  const operations=[],context={beginPath:()=>operations.push('beginPath'),moveTo:()=>operations.push('moveTo'),lineTo:()=>operations.push('lineTo'),stroke:()=>operations.push('stroke'),arc:()=>operations.push('arc'),fill:()=>operations.push('fill')};
  drawTopDownGSensor(context,{longitudinalG:1,lateralG:.5},100,80);
  assert.equal(operations.filter(x=>x==='moveTo').length,2); assert.equal(operations.filter(x=>x==='lineTo').length,2); assert.equal(operations.filter(x=>x==='arc').length,1); assert.equal(operations.filter(x=>x==='stroke').length,1); assert.equal(operations.filter(x=>x==='fill').length,1);
});
...
for (const [profile, createVehicle, presentationKind] of [[FERRARI_TESTAROSSA_VEHICLE_PROFILE,createTestCar,'car'],[HONDA_VFR750R_VEHICLE_PROFILE,createTestBike,'bike']]) {
  for (const side of ['LEFT','RIGHT']) {
    test(`${profile.id} commits the current M7.2 ${side} fork and keeps physics/rendering alive`,()=>{
      ...
      for(let tick=0;tick<1800;tick++){
        const runtimeBefore=resolveLiveRouteTravelerRuntime(live,traveler); const desiredL=runtimeBefore.packageId==='CONTENT_STAGE_1'?sampleLiveRouteChoiceTargetL(live,traveler,choiceId,car.course.s):0;
        const input=sampleRivalDrivingInput(runtimeBefore.coordinateFrame,car,desiredL); updateTestVehicle(runtimeBefore.coordinateFrame,runtimeBefore.heightProfile,runtimeBefore.surfaceMap,car,input,DT);
        const recovered=updateM5Recovery(recovery,runtimeBefore.coordinateFrame,runtimeBefore.heightProfile,runtimeBefore.surfaceMap,car,DT,undefined,pendingRouteStageRecoveryTarget(traveler.handoffState,8)); const world={x:car.x,z:car.z};
        if(recovered!==null){resyncLiveRouteTraveler(live,traveler,world);continue;} const routeUpdate=advanceLiveRouteTraveler(live,traveler,world); if(routeUpdate.committed){car.course={...traveler.handoffState.coordinate};committed=true;}
        const runtimeAfter=resolveLiveRouteTravelerRuntime(live,traveler),camera=updateM5Camera(cameraRig,runtimeAfter.coordinateFrame,runtimeAfter.heightProfile,car,CAMERA_PROFILE,DT);
        if(committed){minSpeedAfterCommit=Math.min(minSpeedAfterCommit,car.speed);renderM5Driving(new SoftwareSurface(320,240,new Uint32Array(320*240)),runtimeAfter.selectFarBackground(camera.s),guideCoordinateCurve(runtimeAfter.coordinateFrame),camera,car,runtimeAfter.terrainProfile,runtimeAfter.groundProfile,runtimeAfter.worldSprites,assets,presentationKind,runtimeAfter.roadView??undefined);renderedAfterCommit++;}
        if(committed&&car.course.s>120&&renderedAfterCommit>=30)break;
      }
      assert.equal(committed,true);assert.ok(renderedAfterCommit>=30);assert.ok(car.course.s>120);assert.ok(minSpeedAfterCommit>8,`post-COMMIT speed fell to ${minSpeedAfterCommit}`);assert.equal(recovery.recoveries,0,`${profile.id} ${side} fork must not use recovery as hidden path-following authority`);
    });
  }
}

test('boot and every course root keep route selection at the composition boundary', async () => {
  const [boot,linear,branching,circuit]=await Promise.all([readFile(new URL('../src/boot.ts',import.meta.url),'utf8'),readFile(new URL('../src/main-linear.ts',import.meta.url),'utf8'),readFile(new URL('../src/main.ts',import.meta.url),'utf8'),readFile(new URL('../src/main-circuit.ts',import.meta.url),'utf8')]);
  assert.match(boot,/selectBrowserCourseMode/);assert.match(boot,/browserCourseModeForKey/);assert.match(boot,/location\.assign/);assert.doesNotMatch(linear,/RouteDag|CircuitTopology|routeKind\s*===/);assert.doesNotMatch(branching,/routeKind\s*===/);assert.doesNotMatch(circuit,/routeKind\s*===/);
});
