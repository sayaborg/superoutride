...
function setup() {
  ...
  return createThirdLiveSuccessorRuntime(guide, parentShared(guide), createSpriteAssets());
}

test('live route has independent third stages on both LEFT and RIGHT paths', () => {
  const live = setup();
  assert.deepEqual(
...
test('promotes the old RIGHT goal geometry into STAGE_3_R and generates a distinct new GOAL_R chart', () => {
  const live = setup();
...
test('S3R_CONTINUE resolves physical handoff, new GOAL_R package binding and terminal FINISH', () => {
  const live = setup();
...
test('extends RIGHT through chain + fragments without renderer or browser topology logic', async () => {
