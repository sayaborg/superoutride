...
test('catalog preserves model identifier specification and period as separate fields', () => {
  ...
  assert.deepEqual(
    VEHICLE_CATALOG.map((entry) => [
      ...
      entry.period,
      BROWSER_VEHICLE_KEYS[entry.profile.id],
    ]),
    expected,
  );
  ...
  assert.equal(new Set(Object.values(BROWSER_VEHICLE_KEYS)).size, 9);
});
...
test('browser owns shortcuts while catalog owns explicit presentation family', () => {
  assert.deepEqual(
    BROWSER_VEHICLE_PROFILES.map(({ code, profile }) => [code, profile.id]),
    VEHICLE_CATALOG.map(({ profile }) => [BROWSER_VEHICLE_KEYS[profile.id], profile.id]),
  );
  for (const entry of VEHICLE_CATALOG) {
    assert.equal(browserVehicleProfileForKey(BROWSER_VEHICLE_KEYS[entry.profile.id]), entry.profile);
