...
function render(): void {
  ...
  ctx.fillText(`M8.1 TRAVEL STEER ${M7_1_DEV_COURSE_MODE.routeKind} / ${vehicleKind === 'car' ? 'CAR' : 'MOTORCYCLE'} SWITCH [V] RECOVER [R]`, 8, 23);
...
function switchVehicleAtSafeSpawn(): void {
  ...
  if (vehicleKind === 'car') {
    vehicle = createM5Bike(guide, height, surfaces, s, l, speed);
    vehicleKind = 'bike';
  } else {
    vehicle = createM5Car(guide, height, surfaces, s, l, speed);
    vehicleKind = 'car';
  }
