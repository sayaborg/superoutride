...
export function updateM5Car(
  ...
  if (after.material.supported) {
    ...
    if (car.supported) car.verticalSpeed = Math.min(car.verticalSpeed, 0);
    car.supported = false;
...
function integrateGroundDynamics(
  ...
  if (car.longitudinalSpeed < 0) {
    ...
    if (input.brake) car.lateralSpeed *= 0.9;
  }
...
function integrateUnsupportedPlanar(car: M5CarState, dt: number, profile: CarPhysicsProfile): void {
