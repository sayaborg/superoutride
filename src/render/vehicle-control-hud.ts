...
export function formatVehicleControlHud(
  ...
  const driveTorque = Math.round(Math.max(0, control.deliveredDriveTorque));
  ...
  return {
    ...
    pedals: `ACT T${control.throttleActuator.toFixed(2)} B${control.brakeActuator.toFixed(2)} DRV ${driveTorque.toString().padStart(4)}Nm BRK ${frontBrake}/${rearBrake}Nm${locks}`,
