...
export function createCourseGroundSource(data: CourseGroundSourceData) {
  if (!data || !data.partition || !Array.isArray(data.regions) || !Array.isArray(data.stamps))
    throw new TypeError('Ground source requires compiled composition data');
  const bindings = new Map(data.regions.map((binding) => [binding.region, binding]));
  const sampleInChart = (s: number, l: number, sourceLateralOrigin: number): number => {
    ...
    const region = courseRegionAt(data.partition, s, l, sourceLateralOrigin);
    if (region) {
      const binding = bindings.get(region);
      if (!binding) throw new Error('Compiled Region has no appearance binding');
      let index = 0;
