...
export interface ChildVisualIdentity {
  readonly leftFarBackground: FarBackground;
  readonly rightFarBackground: FarBackground;
}

/**
 * Child visual identity is ordinary runtime content. The renderer never receives a route side;
 * after the validated handoff it simply draws the FarBackground owned by the active package.
 */
export function createChildVisualIdentity(): ChildVisualIdentity {
