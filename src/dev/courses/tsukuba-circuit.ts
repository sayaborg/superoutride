...
export const TSUKUBA_COURSE_2000_LENGTH_METERS = 2_045;
...
export const TSUKUBA_SESSION_CONFIGURATION = compileSessionConfiguration({ rivalCount: 1 });

export const TSUKUBA_PLAYER_RECOVERY_PROFILE: Readonly<RecoveryProfile> = Object.freeze({
  ...RECOVERY_PROFILE,
  targetL: TSUKUBA_PLAYER_START_L,
});

export const TSUKUBA_RIVAL_RECOVERY_PROFILE: Readonly<RecoveryProfile> = Object.freeze({
  ...RECOVERY_PROFILE,
 ...
 */
export function createTsukubaCourse2000Lap(): TsukubaCourse2000Lap {
  ...
  appendStraight(TSUKUBA_HOME_STRAIGHT_LENGTH_METERS);
  ...
  appendStraight(TSUKUBA_BACK_STRAIGHT_LENGTH_METERS);
  ...
  if (Math.hypot(turtle.x, turtle.z) > 1e-7) {
    throw new Error('Tsukuba Course 2000 authoring failed to close');
  }
  if (Math.abs(turtle.chainage - TSUKUBA_COURSE_2000_LENGTH_METERS) > 1e-7) {
...
function createTsukubaHeightProfile(courseLength: number): HeightProfile {
...
const TSUKUBA_EDGE_MARKINGS: readonly LongitudinalRoadMarking[] = Object.freeze([
  Object.freeze({
    centerL: -TSUKUBA_ROAD_HALF_WIDTH_METERS + 0.15,
    width: 0.15,
...
export function createTsukubaCourse2000Runtime(): CircuitLiveRuntime {
  const authored = createTsukubaCourse2000Lap();
  const topology = compileCircuitTopology('DEV__3_TSUKUBA_COURSE_2000', authored.raster);
  const lapLength = topology.lapLength;
  const height = createTsukubaHeightProfile(lapLength);
  const visual = new VisualProfile(lapLength, [
    {
      ...
      name: 'TSUKUBA COURSE 2000',
    },
  ]);
  const surface = createTsukubaSurfaceMap(lapLength);

  return compileCircuitLiveRuntime(
    topology,
    0,
    {
      lMax: TSUKUBA_GROUND_HALF_WIDTH_METERS + 1,
      mMin: 0.25,
      dCam: CURRENT_CAMERA_DISTANCE_METERS,
    },
    { height, visual, surface },
    {
      id: 'DEV__3_TSUKUBA_COURSE_2000_THREE_LAP_RACE',
      lapCount: 3,
