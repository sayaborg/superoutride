...
`src/main-linear.ts`, `src/main.ts` and `src/main-circuit.ts` assemble shipped DEV courses for browser play. The declared `build:ground` entry consumes the same course authoring to generate product color assets. General engines never import DEV, and shipped courses never import fixtures or diagnostics. [Repository hygiene](../../tests/infrastructure/repository-hygiene.test.mjs) enforces dependency direction, descriptive naming and a test/tool consumer for every regression source. Fixture availability alone never makes an unreachable general module live.
