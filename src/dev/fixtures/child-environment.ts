...
export interface ChildEnvironmentContent {
  readonly left: ChildEnvironment;
  readonly right: ChildEnvironment;
}

/**
 * Package-owned post-handoff scenery. The first 60 child meters stay at the shared height
 ...
 */
export function createChildEnvironmentContent(
  continuation: ChildStageContinuation,
  assets: SpriteAssets,
): ChildEnvironmentContent {
  if (!(continuation.handoffLocalS + 5 < SHARED_FLAT_END_S)) {
    throw new Error('child scenery must begin after the D_cam handoff neighborhood');
  }
...
function createCoastEnvironment(source: ChildStageRuntimeSource, assets: SpriteAssets): ChildEnvironment {
...
function createMountainEnvironment(source: ChildStageRuntimeSource, assets: SpriteAssets): ChildEnvironment {
  const heightProfile = new HeightProfile(source.guide.length, [
    { s: 0, y: 0 },
    { s: SHARED_FLAT_END_S, y: 0 },
    { s: 105, y: 4 },
